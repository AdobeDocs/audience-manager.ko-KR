...
| 주제 | 설명 |
|---- |----|
| [Audience Manager의 접근성](/help/using/overview/accessibility-in-aam.md) | 접근성이란 시각, 청각, 인지, 모터 또는 기타 장애와 같은 다양한 장애가 있는 사용자가 가능한 한 적은 노력으로 소프트웨어 제품을 사용할 수 있도록 하는 일련의 기능을 의미합니다. 모든 사람이 Audience Manager을 쉽게 사용할 수 있도록 이 문서에 설명된 여러 접근성 기능에 대한 지원을 개발했습니다. |
...
| [새 SFTP 서버](../faq/faq-data-collection.md) | 이제 [!DNL SFTP]에 대해 [!DNL ftp-in-gtw]을(를) 사용하는 새 [!DNL ftp-out-gtw] 서버([!DNL AWS SFTP Gateway] 및 [!DNL S3])를 사용할 수 있습니다. |
...
| 주제 | 설명 |
|---- |----|
| [Audience Manager와 Adobe Experience Platform 간의 대상 공유](/help/using/integration/integration-aep/aam-aep-audience-sharing.md#aep-aam-segment-population-differences) | 새 섹션 **Audience Manager과 Experience Platform 간의 세그먼트 모집단 차이점 이해**&#x200B;에서는 Experience Platform과 Audience Manager 간에 모집단 숫자가 다를 수 있는 가능한 이유를 설명합니다. |
| [Audience Manager REST API 시작](/help/using/api/rest-api-main/aam-api-getting-started.md#jwt) | **을(를) 사용하여 새 섹션[!DNL Adobe I/O]** JWT(서비스 계정) 인증을 추가했습니다. [!DNL Adobe I/O]을(를) 사용하여 Audience Manager API를 인증하려면 이 섹션을 참조하십시오. |
...
| [IAB TCF를 위한 Audience Manager 플러그인](../overview/data-security-and-privacy/aam-iab-plugin.md) | Privacy by Design에 주력한 Adobe은 IAB TCF용 Audience Manager 플러그인을 2020년 6월 10일부터 IAB TCF(Transparency &amp; Consent Framework) 버전 2.0으로 업그레이드했습니다. IAB TCF용 Audience Manager 플러그인을 구현한 고객이 이 기능을 계속 사용하려면 2020년 8월 15일까지 버전 2.0으로 업그레이드해야 합니다. 2020년 8월 15일 이후에는 버전 1.1이 더 이상 지원되지 않습니다. |
...
| 주제 | 설명 |
|---- |----|
| [Predictive Audiences](../features/algorithmic-models/predictive-audiences.md) | [!UICONTROL Predictive Audiences]을 사용하면 알 수 없는 대상자를 고급 데이터 과학 기술을 사용하여 실시간으로 개별 성향으로 분류할 수 있습니다. <br><br> 마케팅 컨텍스트에서 성향은 인구 통계, 탐색 습관, 쇼핑 이력 등과 같은 특정 트레이트 집합을 공유하는 방문자, 사용자 또는 잠재적 구매자로 정의되는 대상자 세그먼트입니다.<br><br>[!UICONTROL Predictive Audiences] 모델은 Audience Manager의 기계 학습 기능을 사용하여 알 수 없는 대상을 개별 성향으로 분류할 수 있도록 함으로써 이러한 개념을 한 단계 더 발전시킵니다. <br><br>Audience Manager를 사용하면 일단의 알려진 자사 고객에 대해 알 수 없는 자사 대상의 성향을 계산하여 이를 달성할 수 있습니다. |
