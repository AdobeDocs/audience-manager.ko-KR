...
| 주제 | 설명 |
|--- |----|
| [CCPA(캘리포니아 소비자 개인 정보 보호법) 지원 및 개인 정보 보호 설명서 정비](../overview/data-security-and-privacy/data-privacy.md) | 다음 [CCPA(캘리포니아 소비자 개인 정보 보호법)](https://www.caprivacy.org/about)2020년 1월 1일부터 시행되는 은 캘리포니아 주민에게 개인 정보에 대한 새로운 권리를 제공하고 캘리포니아에서 사업을 하는 특정 업체에 데이터 보호 책임을 부과합니다. <br> <br> Audience Manager은 다음과 같은 개인 정보 보호 도구를 통해 개인 정보 보호 규정에 따른 귀하의 의무를 준수하도록 도와줍니다. [Adobe Experience Platform Privacy Service](https://www.adobe.io/apis/experienceplatform/home/services/privacy-service.html) 데이터 액세스 및 삭제 요청에 사용됩니다. <br><br> 현재 항목이 업데이트되었습니다. [옵트아웃 관리](../overview/data-security-and-privacy/data-privacy-requests.md#opt-out-requests) 선언된 ID(예: CRM ID)를 옵트아웃하는 것을 포함하는 프로세스입니다. 선언된 ID로 옵트아웃하는 경우 선언된 ID 및 마지막으로 연결된 장치가 Audience Manager 데이터 수집에서 옵트아웃됩니다. 이제 옵트아웃 요청도 세그먼트 해제 요청을 (으)로 보냅니다. [대상 파트너](../overview/data-security-and-privacy/assets/AAM-Partners-October2019.xlsx) 배치 및 실시간으로 이 기능을 지원합니다. <br><br> 또한, 우리는 우리의 [데이터 보안](../overview/data-security-and-privacy/data-security.md), [데이터 개인 정보 보호](../overview/data-security-and-privacy/data-privacy.md), 및 [데이터 거버넌스](../overview/data-security-and-privacy/data-governance.md) 설명서를 통해 앞서 언급된 규정을 준수하는 데 필요한 정보를 보다 쉽게 찾을 수 있습니다. |

## 2019년 10월 {#aam-october-updates}

| 주제 | 설명 |
|--- |----|
| [Adobe Sensei 제공, Audience Marketplace 데이터에 대한 지능형 추천](../features/segments/trait-recommendations.md) | Trait Recommendations을 사용하여에서 세그먼트를 작성하거나 편집할 때 [세그먼트 빌더](../features/segments/segment-builder.md), 이제 포함할 수 있는 추가 트레이트에 대한 추천을 받습니다. [!UICONTROL Audience Marketplace] 구독하지 않은 데이터 피드. 세그먼트에 권장 트레이트를 추가하여 타겟 대상자를 늘리십시오. <br>  또한 [!UICONTROL Marketplace] 유사한 트레이트를 쉽게 찾고 데이터 피드를 필터링할 수 있도록 해 주는 페이지입니다. |
...
| [프로필 병합 규칙 개선 사항](../features/profile-merge-rules/merge-rules-overview.md) | 에 대한 일련의 개선 사항이 릴리스되었습니다 [!UICONTROL Profile Merge Rules]: <ul><li>이제 최대 100개의 장치에 대해 세그먼트 평가가 일괄적으로 지원됩니다.</li><li>트레이트 및 세그먼트 모집단에 대한 보고 정확도를 개선했습니다.</li><li>장치 간 ID를 사용하여 생성된 일괄 처리 파일의 정확도를 개선했습니다.</li><li>각 규칙에 대한 자세한 사용 사례와 함께 설명서를 업데이트했습니다. 다음을 참조하십시오 [프로필 병합 규칙에 대한 일반 사용 사례](../features/profile-merge-rules/merge-rule-targeting-options.md), [외부 장치 그래프 사용 사례](../features/profile-merge-rules/external-graph-use-cases.md), 및 [프로필 링크 장치 그래프 사용 사례](../features/profile-merge-rules/profile-link-use-case.md).</li></ul> |
...
| [twitter 사용자 지정 대상을 셀프서비스 장치 기반 대상으로 구성](../features/destinations/twitter-tailored-audiences.md) | twitter 대상을 셀프 서비스 구성 모델로 마이그레이션하고 있습니다. 이 문서에서는 기존 Twitter 통합에서 마이그레이션 후 작업을 계속 진행하기 위해 수행해야 하는 작업에 대해 설명합니다. |
...
| 주제 | 설명 |
|--- |----|
| [AUDIENCE MANAGER API](/help/using/api/rest-api-main/rest-api-main.md) | 진행 중입니다. [모든 API 마이그레이션](/help/using/api/api-swagger-migration.md) 제품 설명서에서 Swagger API로 복사합니다. 마이그레이션한 최신 두 개의 API는 다음과 같습니다. <ul><li>[알고리즘 모델 API](https://bank.demdex.com/portal/swagger/index.html#/Algorithmic_Models_API)</li><li>[파생 신호 API](https://bank.demdex.com/portal/swagger/index.html#/Derived_Signals_API)</li></ul> |
...
| [크로스 디바이스 데이터 소스 만들기](/help/using/features/profile-merge-rules/merge-rules-start.md#settings) | 장치 간 데이터 소스를 만들거나 편집하기 위해 워크플로에 새 컨트롤을 도입했습니다. **비활성 고객 ID에 대한 데이터 유지**. 이제 비활성 고객 ID에 대한 데이터 보존 기간을 설정하여 Audience Manager 플랫폼에서 고객 ID를 마지막으로 본 후 Audience Manager이 데이터베이스에 고객 ID를 유지하는 기간을 결정할 수 있습니다. |
