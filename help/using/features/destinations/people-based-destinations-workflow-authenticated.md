...
이 페이지에는 오프라인 [!DNL CRM] 데이터와 인증된 사용자가 고객 세그먼트를 만든 다음 이 고객 세그먼트를 로 전송하는 실시간 행동 데이터를 결합하는 방법에 대한 단계별 지침이 포함되어 있습니다 [!DNL People-Based Destinations].
...
이 경우 5단계 - [사용자 기반 Platform 인증 구성으로 건너뜁니다](#configure-authentication).
...
인증된 사용자가 규칙 기반 트레이트를 이용할 수 있도록 자격 조건을 부여하려면 [선언된 ID를 통해 트레이트 자격 조건을 전송해야 합니다](../declared-ids.md).
...
>[!IMPORTANT]
>
>이미 **[!UICONTROL Current Authenticated Profiles]** 또는 **[!UICONTROL Last Authenticated Profiles]** 옵션으로 정의된 규칙이 있는 경우 4단계 - 대상자 세그먼트 [만들기로 건너뛸 수 있습니다](#create-audience-segments).
...
4. 섹션의 **[!UICONTROL Profile Merge Rule Setup]** 목록에서 **[!UICONTROL Current Authenticated Profiles]** 또는 **[!UICONTROL Last Authenticated Profiles]** 규칙을 **[!UICONTROL Cross-Device Options]** 선택합니다.
5. 목록에서 **[!UICONTROL Cross-Device Profile Options]** 세그멘테이션을 실행할 데이터 소스를 선택합니다. 기존 DPUUID가 포함된 데이터 소스여야 합니다.
   ![병합 규칙 설정](assets/pbd-pmr-combined.png)

## 4단계 - 대상 세그먼트 만들기 {#create-audience-segments}

새 세그먼트를 만들려면 세그먼트 빌더를 [사용하십시오](../segments/segment-builder.md). 보내려는 기존 대상 세그먼트가 있는 경우 5단계 [!DNL People-Based Destinations]- 사용자 기반 Platform 인증 [구성으로 건너뜁니다](#configure-authentication).
