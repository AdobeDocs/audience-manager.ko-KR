...
이 문서는에 보낼 수 있는 개별 데이터 개인 정보 보호 및 옵트아웃 요청 관리에 대한 개요를 제공합니다 [!DNL Audience Manager] 다음을 통해 [PRIVACY SERVICE UI](https://privacyui.cloud.adobe.io/) 및 **[!DNL Privacy Service API]**.

이러한 도구를 사용하면 아래에 수행된 소비자 데이터 개인 정보 보호 요청을 보낼 수 있습니다. [!DNL GDPR] 및 [!DNL CCPA].
...
에서 소비자 데이터에 액세스하고 삭제하기 위한 개별 요청을 제출할 수 있습니다. [!DNL Audience Manager], 두 가지 방법으로:
...
개별 데이터 개인 정보 보호 요청을 보낼 때 다음을 제출할 수 있습니다. [!DNL Audience Manager] 에 설명된 대로 식별자(ID) **[Audience Manager 식별자](data-privacy-ids.md)** 섹션에 해당 네임스페이스 ID(데이터 소스 ID)를 추가합니다.
...
다음을 통해 데이터 삭제 요청을 보낼 수 있습니다. [PRIVACY SERVICE UI](https://privacyui.cloud.adobe.io) (설명서) [여기](https://experienceleague.adobe.com/docs/experience-platform/privacy/ui/overview.html?lang=en) 또는 Privacy Service API를 호출하여(설명서) [여기](https://experienceleague.adobe.com/docs/experience-platform/privacy/api/overview.html?lang=en) 및 [!DNL API] 참조 [여기](https://developer.adobe.com/experience-platform-apis/references/privacy-service/).

[Privacy Service UI](https://privacyui.cloud.adobe.io/)를 사용하면 [!UICONTROL Request Builder]를 사용하거나 [!DNL JSON] 파일을 업로드하여 새 작업 요청을 만들 수 있습니다.

유효한 [!DNL JSON] 파일의 모습을 보기 위해 [샘플 JSON을 다운로드](../data-security-and-privacy/assets/access_request.json)할 수 있습니다.

Adobe는 30일 이내에 데이터 개인 정보 보호 고객 요청을 이행하기 위한 귀하의 노력을 이해합니다. 그런 이유로, [!DNL Adobe] 는 가능한 한 빨리 데이터 삭제 요청을 처리하는 데 전념합니다.

소비자 데이터 삭제 요청에 대한 응답으로, [!DNL Audience Manager] 와(과) 연결된 트레이트 및 세그먼트를 삭제합니다. [!DNL Audience Manager] 요청에 포함된 식별자. 또한 각각 [!DNL Audience Manager] 에 의해 추가 데이터 수집을 옵트아웃한 개인의 식별자 [!DNL Audience Manager] 및 해당 ID 매핑이 제거됩니다.

교차 장치와 같이 선언된 ID를 보낼 때 [!DNL CRM] ID 또는 [!DNL cookie] ID, 데이터 개인 정보 보호 요청 시 [!DNL Audience Manager] 는 모든 연결된 장치(선언된 ID당 최대 100개의 장치)에서 필요한 삭제를 수행합니다.

[!DNL Audience Manager] 은(는) 특정 데이터의 삭제를 요청하는 데이터 주체에 대한 세그먼트 해제 정보를 보내어 활성화 파트너에게 삭제 요청에 대해 알리려고 합니다. 그러나 일부 활성화 파트너는

1. 에서 세그먼트 해제(또는 세그먼트 제거) 요청을 지원할 수 없음 [!DNL Audience Manager] 및/또는
2. 에서 업데이트를 받을 수 없음 [!DNL Audience Manager] 빈도가 30일 미만인 경우 그런 경우에는, [!DNL Audience Manager] 고객은 를 통해 자동화된 방식으로 활성화 파트너에게 삭제 요청을 보낼 수 없습니다. [!DNL Audience Manager].

이러한 경우 를 통해 자동화된 방식으로 활성화 파트너에게 삭제 요청을 보낼 수 없습니다. [!DNL Audience Manager].

다음 참조: [장치 기반 대상 목록 설명서](assets/AAM-Partners-October2019.xlsx) 확인 [!DNL Audience Manager] 활성화 파트너는 세그먼트 해제를 지원합니다.

## 옵트아웃 요청 {#opt-out-requests}

[!DNL Audience Manager] 은 옵트아웃 관리와 관련된 업계 표준을 지원합니다. 에서 지원하는 옵트아웃 유형에 대한 자세한 내용은 을 참조하십시오 [!DNL Audience Manager].

데이터 액세스 및 삭제 요청은 [Privacy Service](https://experienceleague.adobe.com/docs/experience-platform/privacy/home.html?lang=en)를 통해 처리되는 반면 옵트아웃 요청은 현재 [!DNL DCS API]를 통해 지원됩니다. 옵트아웃에 대해 알아보려면 계속 읽기 [!DNL API] 호출은 다음과 같아야 합니다.

### 글로벌 옵트아웃 요청

전역 옵트아웃은 의 옵트아웃을 나타냅니다. [!DNL Audience Manager] 및 기타 [!DNL Adobe Experience Cloud] 모든 브랜드를 위한 솔루션 아래 표에는 글로벌 옵트아웃에 사용되는 방법이 나와 있습니다.
...
* [!DNL Audience Manager] 사용자가 브라우저 쿠키를 지우지 않는 한 모든 데이터 수집, 세그먼테이션 또는 활성화를 중단합니다.
* 이전 데이터는 120일 후 사용자 프로필에서 제거됩니다.

### 선언된 ID 호출을 사용한 파트너 수준 옵트아웃

파트너 수준 옵트아웃을 사용하면 사용자에 따라 데이터 수집에서 옵트아웃할 수 있습니다 [!DNL Audience Manager] 파트너. 다음을 포함한 교차 장치 ID에 대한 파트너 수준 옵트아웃 요청을 보낼 수 있습니다. [!DNL CRM] ID 및 해시된 이메일 주소.
...
* [!DNL Audience Manager] 에 대한 모든 데이터 수집, 세분화 또는 활성화를 중단합니다. [!DNL CRM] ID와 연결된 마지막 장치 ID [!DNL CRM] ID;
* [!DNL Audience Manager] 옵트아웃의 세그먼트를 해제합니다. [!DNL CRM] 모든 세그먼트의 ID 및 마지막 장치 ID;
* [!UICONTROL Destination] 파트너는 다음에 대한 세그먼트 해제 요청을 받습니다. [!DNL CRM] ID 및 마지막 장치 ID. 세그먼테이션 해제는 [실시간](data-privacy-requests.md#aam-partners-with-unsegmentation) 대상과 배치 대상 모두에 대해 작동합니다.
* 이전 데이터는 삭제되지 않습니다.

날짜 [!DNL Audience Manager] 은 파트너 수준 옵트아웃 요청을 수신합니다. [!DNL JSON] 에 의해 반환됨 [!DNL DCS] 다음을 포함: [오류 코드 171](../../api/dcs-intro/dcs-api-reference/dcs-error-codes.md#opt-out-error-codes), 메시지 포함 [!UICONTROL "Encountered opt out tag"], 대신 [!DNL Audience Manager] 사용자 ID.
...
| 복수  `d_cid`  및  `d_cid_ic`  키-값 쌍입니다. | `https://yourcompany.demdex.net/demoptout?d_cid=123%01987&d_cid_ic=456%01321...` |

 

### 장치 ID 호출을 사용한 파트너 수준 옵트아웃

파트너 수준 옵트아웃을 사용하면 사용자에 따라 데이터 수집에서 옵트아웃할 수 있습니다 [!DNL Audience Manager] 파트너. [DCS API](../../api/dcs-intro/dcs-api-reference/dcs-api-reference-overview.md)를 다음과 같이 호출하여 브랜드의 특정 기기 ID에서 데이터 수집을 옵트아웃할 수 있습니다.
...
* [!DNL Audience Manager] 은(는) 앞으로 장치 ID에 대해 파트너의 모든 데이터 수집, 세그멘테이션 또는 활성화를 중단합니다.
...
소비자 데이터 개인 정보 보호 요청을 자동화하는 데 도움이 되도록 [!DNL Audience Manager] 은(는) 세그먼트 해제(또는 세그먼트 제거) 정보를 보내어 활성화 파트너에게 데이터 주체의 삭제 요청에 대해 알리려고 합니다.

그러나 일부 활성화 파트너는

1. 에서 세그먼트 해제 요청을 지원할 수 없음 [!DNL Audience Manager] 및/또는
2. 에서 업데이트를 받을 수 없음 [!DNL Audience Manager] 30일 동안 한 번 이상
...
주어진 [!DNL Audience Manager] 은(는) 데이터의 소스가 아니며 에서 데이터 수정에 대한 제한된 역할이 있습니다. [!DNL Audience Manager]. 수정은 소비자가 잘못된 항목에서 자격을 박탈하라고 요청했음을 의미할 수 있습니다 [!UICONTROL trait]/[!UICONTROL segment] 또는 원하는 자격을 갖춤 [!UICONTROL trait]/[!UICONTROL segment].

[!DNL Audience Manager] 고객은 사용자 프로필과 관련된 신호/트레이트/세그먼트를 캡처하고 이 정보를 다음을 통해 보내도록 선택할 수 있습니다. [오프라인 데이터 수집](../../integration/sending-audience-data/batch-data-transfer-explained/batch-data-transfer-overview.md) 끝 [!DNL Audience Manager]. 사용자는 계속해서 원본에 대한 자격을 갖습니다. [!UICONTROL trait] 및 [!UICONTROL segments] 만약 그들이 그들의 행동을 반복한다면
