...
# [!UICONTROL Trait]의 [!UICONTROL Segment] 및 [!UICONTROL Segment Builder] 모집단 데이터 {#trait-and-segment-population-data-in-segment-builder}

실제 및 예상 세그먼트 모집단 데이터와 함께 실제 [!UICONTROL traits] 모집단을 보려면 [!UICONTROL Segment Builder]에서 [!UICONTROL trait]을(를) 추가하고 제거하십시오. 예상 모집단 크기 데이터를 사용하면 캠페인에 적합한 세그먼트를 만드는 데 도움이 됩니다.

## [!UICONTROL Trait] 모집단 데이터 {#trait-population-data}

[!UICONTROL Segment Builder]은(는) 세그먼트에 [!UICONTROL Total Trait Population]을(를) 추가하는 마지막 날의 [!UICONTROL trait]을(를) 표시합니다. 이 데이터는 [!UICONTROL trait] 섹션에서 선택한 [!UICONTROL Basic View] 주변의 파란색 필드에 표시됩니다.
...
하지만 Audience Manager은 세그먼트의 실시간 및 총 인구 크기를 즉시 예측할 수 있습니다. 이러한 추정치는 샘플링된 내역 데이터 및 95% 신뢰 구간에서 반환된 결과를 기반으로 합니다.
...
<table id="table_B24503F372E34B6BBDF5204181701A59"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>지정된 시간 범위 동안 실시간으로 확인되었으며 Audience Manager에서 확인한 순간 세그먼트에 대한 자격이 확인된 예상 고유 방문자 수입니다. </p> <p><span class="wintitle"> 세그먼트 빌더</span>에서 트레이트(<span class="wintitle"> 총 트레이트 인구</span>)에 대한 마지막 30일 인구는 실시간으로 평가되는 트레이트 및 세그먼트에 대해 다를 수 있습니다. </p> <p>
     <ul id="ul_CAE803D09913462CAD413A665D85C1A2"> 
      <li id="li_3E64330D6F5B4D2F8F3456730A86894F">트레이트의 경우 지난 30일 지표는 지난 30일 동안 해당 트레이트에 대한 자격이 있는 고유 사용자의 수를 계산합니다. </li>
      <li id="li_FF3CACD0B6C742CDB94D66200D77CE06">실시간으로 평가된 세그먼트의 경우, 마지막 30일 지표는 과거 어느 시점에서 트레이트(해당 세그먼트의)에 대한 자격이 있고 지난 30일 이내에 Audience Manager에서 다시 본 사용자의 수를 계산합니다. 예를 들어 60일 전에 트레이트에 대한 자격이 있고 10일 전에 다시 조회된 사용자가 있다고 가정해 보겠습니다. 데이터에서 이 사용자는 30일 이상 전에 트레이트에 대한 자격이 처음 있으므로 트레이트 수에 추가되지 않습니다. 하지만 실시간으로 평가되는 세그먼트의 마지막 30일 수에 포함됩니다. 이는 30일 시간 간격 내에 세그먼트에 대한 자격이 있기 때문입니다. </li>
     </ul> </p> <p> <p>참고: <span class="wintitle"> 예상 실시간 모집단</span> 지표에는 <span class="wintitle"> 장치 그래프 옵션</span>을(를) 사용하는 <a href="../../features/profile-merge-rules/merge-rule-definitions.md#device-options"> 프로필 병합 규칙</a>에서 제공한 연결을 기반으로 세그먼트에 적합한 장치가 포함되지 않습니다. </p> </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col2"> <p>새 세그먼트나 수정된 세그먼트에 있을 수 있는 예상 고유 방문자 수입니다. 거의 모든 예측과 마찬가지로 과거 성과가 미래 결과를 보장하지는 않지만 예상 합계를 사용하여 다음을 수행할 수 있습니다. </p> <p> 
     ...
     </ul> </p> <p> <p>참고: <span class="wintitle"> 예상 총 인구</span> 지표에는 <span class="wintitle"> 장치 그래프 옵션</span>을(를) 사용하는 <a href="../../features/profile-merge-rules/merge-rule-definitions.md#device-options"> 프로필 병합 규칙</a>에서 제공한 연결을 기반으로 세그먼트에 적합한 장치가 포함되지 않습니다. </p> </p> </td> 
  </tr> 
 </tbody> 
</table>
...
### [!UICONTROL Merge Rules]이(가) 없는 [!UICONTROL Device Graph Option]의 세그먼트 채우기 데이터

다음 표는 [!UICONTROL Profile Merge Rule] 옵션 없이 만든 [!UICONTROL device graph]에서 세그먼트를 사용할 때의 실제 실시간 및 총 모집단 지표를 정의합니다. 장치 옵션 설정 **[!UICONTROL No Device Options]** 및 **[!UICONTROL Current Device Proflie]**&#x200B;입니다.
...
<table id="table_A18C973855DB46A0B39B81F32E0E7540"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>지정된 시간 범위 동안 실시간으로 확인되었으며 Audience Manager에서 확인한 순간 세그먼트에 대한 자격이 확인된 실제 고유 방문자 수입니다. </p> <p><span class="wintitle"> 세그먼트 빌더</span>에서 트레이트(<span class="wintitle"> 총 트레이트 인구</span>)에 대한 마지막 30일 인구는 실시간으로 평가되는 트레이트 및 세그먼트에 대해 다를 수 있습니다. </p> <p> 
     <ul id="ul_50D1528DDDF347858F17DA3C033B0E3F"> 
      <li id="li_ABA2BFE68FF4430DBB425C4661E1836A">트레이트의 경우 지난 30일 지표는 지난 30일 동안 해당 트레이트에 대한 자격이 있는 고유 사용자의 수를 계산합니다. </li> 
      <li id="li_1519068CBB1445E893657D12E8FE42AC">실시간으로 평가된 세그먼트의 경우, 마지막 30일 지표는 과거 어느 시점에서 트레이트(해당 세그먼트의)에 대한 자격이 있고 지난 30일 이내에 Audience Manager에서 다시 본 사용자의 수를 계산합니다. 예를 들어 60일 전에 트레이트에 대한 자격이 있고 10일 전에 다시 조회된 사용자가 있다고 가정해 보겠습니다. 데이터에서 이 사용자는 30일 이상 전에 트레이트에 대한 자격이 처음 있으므로 트레이트 수에 추가되지 않습니다. 하지만 실시간으로 평가되는 세그먼트의 마지막 30일 수에 포함됩니다. 이는 30일 시간 간격 내에 세그먼트에 대한 자격이 있기 때문입니다. </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
...
### [!UICONTROL Merge Rules] 옵션이 있는 [!UICONTROL Device Graph]에 대한 세그먼트 채우기 데이터

다음 표는 [!UICONTROL Profile Merge Rule] 옵션으로 만든 [!DNL device graph]에서 세그먼트를 사용할 때의 실제 실시간 및 총 모집단 지표를 정의합니다. 사용 가능한 [!UICONTROL Profile Link Device Graph], [!DNL Adobe] [!DNL device graph] 및 기타 타사 [!DNL device graph] 선택 항목에 대한 장치 옵션 설정입니다.


| 열 A | 열 B |
|---------|----------|
| [!UICONTROL Real-Time Population (Existing)] | 장치 그래프로 연결된 최대 100개의 다른 장치 프로필과 병합될 때 Audience Manager에서 본 바로 해당 세그먼트에 적합한 트레이트가 포함된 현재 프로필로 실시간으로 표시되는 실제 장치 수입니다. |
