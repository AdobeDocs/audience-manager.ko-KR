...
| 주제 | 설명 |
|----|----|
| [Predictive Audiences](/help/using/features/algorithmic-models/predictive-audiences.md) | <ul><li>**Predictive Audiences 모델 복제**: 이제 기존 모델을 복제하고 필요에 따라 구성을 변경할 수 있습니다. 자세한 내용은 [예측 대상 모델 복제 및 편집](/help/using/features/algorithmic-models/predictive-audiences.md#clone-predictive-audiences)을 참조하십시오.</li><li>**Predictive Audiences 일괄 분류**: 예측 대상은 실시간으로 개별 성향으로 방문자를 분류하는 것 외에도 배치 분류를 지원하므로 온보드 사용자를 분류하고 배치 대상으로 활성화할 수 있습니다.</li></ul> |
...
| [인바운드 S3 ](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-s3-filenames.md) 파일 이름 및  [인바운드 FTP 파일 이름](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-ftp-filenames.md) | 파일 이름 구문의 `DPID` 및 `DPID_TARGET_DATA_OWNER` 필드에 대한 정의와 예제를 업데이트했습니다. |

## 2020년 8월 {#aam-august-updates}

| 주제 | 설명 |
|---- |----|
| [사용자 기반 대상에 대한 Google Customer Match](../features/destinations/people-based-destinations-prerequisites.md#gcm) | 이제 [!UICONTROL People-Based Destinations]을 사용하여 [!DNL Google Customer Match]을 통해 사용자를 타깃팅할 수 있습니다. [!UICONTROL People-Based Destinations] 을 사용하여 자사 대상 세그먼트를 [!DNL Google Customer Match] 대상에 보내려면 먼저 [!DNL Google]에서 이 사용자를 해당 허용 목록에 추가해야 합니다. 자세한 내용은 [사람 기반 대상](../features/destinations/people-based-destinations-prerequisites.md#gcm) 설명서를 참조하십시오. |
| [새 SFTP 서버](../faq/faq-data-collection.md) | 이제 [!DNL S3]보다 [!DNL AWS SFTP Gateway]를 사용하는 새 [!DNL SFTP] 서버([!DNL ftp-in-gtw] 및 [!DNL ftp-out-gtw])를 사용할 수 있습니다. |
...
| 주제 | 설명 |
|---- |----|
| [Audience Manager와 Adobe Experience Platform 간의 대상 공유](/help/using/integration/integration-aep/aam-aep-audience-sharing.md#aep-aam-segment-population-differences) | 새 섹션인 **Audience Manager과 Experience Platform** 간의 세그먼트 모집단 차이를 이해하면 Experience Platform과 Audience Manager 간에 모집단 수가 다를 수 있는 가능한 이유를 설명합니다. |
| [Audience Manager REST API 시작](/help/using/api/rest-api-main/aam-api-getting-started.md#jwt) | [!DNL Adobe I/O]**을(를) 사용하여** JWT(서비스 계정) 인증 섹션을 새로 추가했습니다. [!DNL Adobe I/O]을 사용하여 Audience Manager API를 인증하려면 이 섹션을 참조하십시오. |
