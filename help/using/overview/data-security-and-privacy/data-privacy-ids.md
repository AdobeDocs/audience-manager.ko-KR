---
description: 이 문서에서는 데이터 개인 정보 요청에 사용할 수 있는 Audience Manager ID 유형을 다룹니다.
seo-description: 이 문서에서는 데이터 개인 정보 요청에 사용할 수 있는 Audience Manager ID 유형을 다룹니다.
...
Adobe Audience Manager에 [데이터 개인 정보 보호 요청을](data-privacy-requests.md) 제출할 때는 아래에 나열된 식별자(ID) 중 하나를 포함해야 합니다. ID 형식에 대한 자세한 내용은 Audience Manager ID [색인에서 확인할 수 있습니다](../../reference/ids-in-aam.md).
...
* **사용자 ID**: `mid`
* **정의**: [!DNL Adobe Experience Cloud ID], 이전에는 [!DNL Visitor ID] 또는 [!DNL Marketing Cloud ID]
* **네임스페이스 ID**:4
...
**정의**:익명의 사이트 방문자에 대해 설정한 쿠키 또는 오프라인 시스템의 ID 또는 해시된 사용자 [!DNL CRM] 이름과 같은 고객 ID입니다.
...
>[!IMPORTANT]
>
> Mobile을 사용하는 [!DNL SDK]경우 Experience Cloud ID(`MID`)를 모바일 광고 ID와 함께 전송하여 전체 액세스 및 삭제 응답을 해야 합니다.
