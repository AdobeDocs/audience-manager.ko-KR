...
모니터링하려는 사이트 섹션의 URL에 캠페인 관련 쿼리 문자열을 추가하십시오. 그런 다음 이 변수를 캡처하도록 트레이트 규칙을 설정합니다. 예를 들어, URL이 `www.test123.com/electronics?campaign=123`과(와) 같이 캠페인 ID를 전달한다면 `h_referer = 'campaign=123'`과(와) 같은 헤더를 찾는 트레이트 규칙이 있는 `h_referer` 변수에서 해당 데이터를 캡처하는 트레이트 규칙을 만듭니다.
