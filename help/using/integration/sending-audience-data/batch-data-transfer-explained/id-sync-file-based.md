...
>[!NOTE]
> 사용자 기반 대상에 대한 ID 동기화 파일 이름 지정(c2c 접두사)에 대해서는 [워크플로 A - 오프라인 데이터와 결합된 모든 온라인 활동을 기반으로 하는 Personalization](../../../features/destinations/people-based-destinations-workflow-combined.md) 또는 [워크플로 B - 오프라인 전용 데이터를 기반으로 하는 Personalization](../../../features/destinations/people-based-destinations-workflow-offline.md)을 참조하십시오.
...
파일에 사용자 ID([!DNL UUID])가 있습니다. 각 행에서 탭으로 ID를 구분합니다. 다음 예제에서는 올바른 형식의 ID 파일을 보여 줍니다. 콘텐츠가 비슷해 보일 수 있습니다.
...
>[!IMPORTANT]
>
>첫 번째 열에서 [DPUUIDs](../../../reference/ids-in-aam.md)을(를) 사용하지 마십시오. 이렇게 하면 동기화가 잘못될 수 있습니다.
