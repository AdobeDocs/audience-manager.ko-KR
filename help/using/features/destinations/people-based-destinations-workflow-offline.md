...
아래 표에서 해당 온보드 트레이트 ID에 대한 고객 ID의 자격을 얻고자 합니다. 사용자의 [DPUUID](../../reference/ids-in-aam.md)가 ID 99999의 데이터 소스에 저장되고 Audience Manager 파트너 ID가 123임을 고려해 보십시오.

| 고객 ID(DPUUID) | 온보드 특성 ID |
| -------------------------------------- | ------------------- |
| 680798276567319850405265607456196039 | 12345, 23456 |
| 674126820834199572553870443620307584 | 45678 |
| 8 | 11223, 93342, 27341 |

<br />

위의 예에서 해당 온보드 트레이트에 대한 고객 ID의 자격을 갖추려면 다음 내용이 포함된 [인바운드 데이터 파일](../../integration/sending-audience-data/batch-data-transfer-explained/inbound-file-contents.md)을 업로드해야 합니다.
...
>[!IMPORTANT]
>
> 이 단계는 위에서 설명한 [시나리오 2](people-based-destinations-workflow-offline.md#configure-data-source-settings)에만 적용됩니다. 기존 [DPUUID](../../reference/ids-in-aam.md)가 이미 해시된 이메일 주소인 경우 [4단계 - 세그멘테이션에 대한 프로필 병합 규칙 만들기](#create-profile-merge-rule)로 건너뜁니다.

1단계의 예에서 아래 표의 해시된 이메일 주소(오른쪽 열)로 기존 [DPUUIDs](../../reference/ids-in-aam.md)와 일치시키고 [2단계 - 데이터 소스 설정 구성](#configure-data-source-settings)에서 만든 새로운 데이터 소스에 이메일 주소를 저장한다고 가정해 봅시다.
...
이 예제에서 [ID 동기화 파일](../../integration/sending-audience-data/batch-data-transfer-explained/id-sync-file-based.md)은 다음과 같은 내용을 갖게 됩니다.
...
## 4단계 - 세그먼테이션 {#create-profile-merge-rule}에 대한 프로필 병합 규칙 만들기
...
1. **[!UICONTROL Segment Mappings]** 섹션에서 이 대상에 보낼 세그먼트를 선택합니다. 이것은 [5단계 - 대상 세그먼트 만들기](people-based-destinations-workflow-offline.md#create-audience-segments)에서 만든 세그먼트입니다.
1. 대상을 저장합니다.
