...
1. [!UICONTROL Cannot be tied to personally identifiable information] 옵션을 선택 취소했는지 확인하십시오.
...
>[!IMPORTANT]
>
> 이 단계는 위에서 설명한 [시나리오 2](people-based-destinations-workflow-combined.md#configure-data-source-settings)에만 적용됩니다. 기존 [DPUUID](../../reference/ids-in-aam.md)가 이미 해시된 이메일 주소인 경우 [3단계 - 세그멘테이션에 대한 프로필 병합 규칙 만들기](people-based-destinations-workflow-combined.md#create-merge-rule)로 건너뜁니다.

기존 [DPUUIDs](../../reference/ids-in-aam.md)를 아래 표의 해시된 이메일 주소(오른쪽 열)와 일치시키고 [1단계 - 데이터 소스 설정 구성](people-based-destinations-workflow-combined.md#configure-data-source-settings)에서 만든 새 데이터 소스에 해시된 이메일 주소를 저장한다고 가정합시다.

| DPUUID(CRM ID) | 이메일 주소 | 해시된 이메일 주소 |
| -------------------------------------- | --------------------- | ---------------------------------------------------------------- |
| 680798276567319850405265607456196039 | `johndoe@example.com` | 55e79200c1635b37ad31a378c39feb120f116625093a19bc32fff150411 49 |
| 674126820834199572553870443620307584 | `janedoe@email.com` | 16d72e3edbeb089b299e0d12fc09522fdc5ece2d11dcb1304ecdd6fab4f7193a |
| 8 | `name@mydomain.com` | feec5debcea411f54462a345a0d9975415d2d4862745ff8af00c49b4ae6 |

 

해시된 이메일 주소를 최대 10개까지 단일 [DPUUID](../../reference/ids-in-aam.md)에 연결할 수 있습니다. 이렇게 하려면 동기화 파일 내에서 해시된 이메일 주소를 쉼표로 구분합니다.
...
ID 동기화 파일을 만든 후에는 [!DNL Amazon S3] 버킷에 업로드해야 합니다. ID 동기화 파일을 업로드하는 방법을 알아보려면 [Audience Manager에 배치 데이터 보내기](../../integration/sending-audience-data/batch-data-transfer-explained/batch-data-transfer-overview.md)를 참조하십시오.

## 3단계 - 세그멘테이션 {#create-merge-rule}에 대한 프로필 병합 규칙 만들기
...
1. **[!UICONTROL Cross-Device Profile Options]** 목록에서 세그멘테이션을 실행할 데이터 소스를 선택합니다. 기존 [DPUUIDs](../../reference/ids-in-aam.md)를 포함하는 데이터 소스입니다.

## 4단계 - 대상 세그먼트 만들기 {#create-audience-segments}

새 대상 세그먼트를 만들려면 [세그먼트 빌더](../segments/segment-builder.md)를 사용하십시오. [!DNL People-Based Destinations]으로 보내려는 기존 대상 세그먼트가 있는 경우 [5단계 - 사용자 기반 플랫폼 인증 구성](people-based-destinations-workflow-combined.md#configure-authentication)으로 건너뜁니다.
...
1. **[!UICONTROL Segment Mappings]** 섹션에서 이 대상에 보낼 세그먼트를 선택합니다. 이것은 [4단계 - 대상 세그먼트 만들기](people-based-destinations-workflow-combined.md#create-audience-segments)에서 만든 세그먼트입니다.
1. 대상을 저장합니다.
