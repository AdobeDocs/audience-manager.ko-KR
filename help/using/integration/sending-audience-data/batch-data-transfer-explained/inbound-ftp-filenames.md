...
seo-description: 데이터를 Audience Manager에 보낼 때 따라야 하는 필수 필드, 구문, 이름 지정 규칙 및 파일 크기에 대해 설명합니다. Audience Manager FTP 디렉토리에 데이터를 보낼 때 이러한 사양에 따라 파일의 이름과 크기를 설정합니다.
...
데이터를 [!DNL Audience Manager]에 보낼 때 따라야 하는 필수 필드, 구문, 이름 지정 규칙 및 파일 크기에 대해 설명합니다. Audience Manager [!DNL FTP] 디렉토리에 데이터를 보낼 때 이러한 사양에 따라 파일의 이름과 크기를 설정하십시오.

>[!WARNING]
>
>[!DNL FTP] 구성에 대한 지원을 단계적으로 중단하고 있습니다. 기존 [!DNL FTP] 통합에서 인바운드 데이터 파일 섭취가 여전히 지원되지만 새로운 통합을 위해 [!DNL Amazon S3] 을 사용하여 오프라인 데이터를 온보딩하는 것이 좋습니다. 자세한 내용은 [인바운드 데이터 파일에 대한 Amazon S3 이름 및 파일 크기 요구 사항](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-s3-filenames.md)을 참조하십시오.
...
<table id="table_1EA97D75004148CE85F702427DB7E97A"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p><span class="keyword"> Audience Manager</span> FTP 디렉토리의 경로 및 이름입니다. FTP 디렉터리 및 자격 증명에 대해서는 계정 관리자에게 문의하십시오. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>정수. 큰 파일을 여러 부분으로 분할할 때 사용하여 처리 시간을 향상시킬 수 있습니다. 숫자는 원본 파일을 보내는 부분을 가리킵니다. </p> <p>효율적인 파일 처리를 위해 표시된 대로 데이터 파일을 분할합니다. </p> 
    ...
    </ul> <p>아래의 처음 2 <a href="../../../integration/sending-audience-data/batch-data-transfer-explained/inbound-ftp-filenames.md#file-name-examples"> 파일 이름 예제</a>를 참조하십시오. </p> </td> 
  </tr> 
  <tr> 
...
[](assets/ftp_dpm_1234_1445374061.overwrite) 추가 예가 필요한 경우 샘플 파일을 다운로드합니다. 이 파일은 `.overwrite` 파일 확장자로 저장됩니다. 간단한 텍스트 편집기로 엽니다.

## 허용되는 파일 크기 {#accepted-file-sizes}

데이터를 [!DNL Audience Manager] / [!DNL FTP] 디렉토리에 보낼 때 파일 크기 제한 및 파일을 가장 빨리 처리하기 위해서는 아래 그림을 고려하십시오.
