...
## DIL을 사용하여 Audience Manager로 데이터 요소 보내기 {#send-data-elements-dil}

페이지 요소에 대한 정보를 Audience Manager로 전송하는 개체 변수를 만듭니다. 이 기능은 일반 데이터 수집 또는 Analytics 변수를 사용하여 데이터를 수집하는 대신 유용합니다.
...
다음 코드는 페이지 데이터를 수집하여 Audience Manager에 보내는 방법을 보여 줍니다. [!UICONTROL DIL] 이러한 예에서는 변수를 사용하여 데이터 요소를 플랫 목록 또는 배열에 보관합니다. 변수를 [키-값 쌍으로](../reference/key-value-pairs-explained.md)전달합니다. 또한 키-값 쌍의 키 앞에 있는 `c_` 접두사를 확인하십시오. 이 [필수 접두사는](../features/traits/trait-variable-prefixes.md) 정보를 사용자 정의 데이터로 식별합니다. 첫 번째 예에서는 키에 수동으로 추가해야 `c_` 합니다. 두 번째 예에서는 자동으로 [!UICONTROL DIL] 수행합니다.

**일관된 값 속성 유지**

데이터를 전달할 때 값 속성을 동일하게 유지해야 합니다. 예를 들어 값이 다른 두 개의 동일한 키가 있는 경우 마지막 키-값 쌍의 값이 이전 값 개체보다 우선합니다. 예를 들어, 전달을 `color:blue` 통해 반환된 값을 빨간색으로 `color:red` 설정합니다(파란색 덮어쓰기).
...
>[!NOTE]
>
>이 메서드는 사용자가 유사한 프로토콜(HTTP와 HTTPS)을 사용하는 페이지 간을 이동할 때만 작동합니다. 예를 들어 보안 사이트에서 다른 보안 사이트로 이동할 때 브라우저는 참조 URL을 유지합니다. 보안 사이트와 비보안 사이트 간을 이동할 때 브라우저는 참조 URL을 유지하지 않습니다. 이 동작은 일반적인 브라우저 기능이므로 이 동작을 우회할 수 없습니다 [!UICONTROL DIL].
...
검색 엔진 유형 및 키워드 검색에 대한 정보를 Audience Manager로 보냅니다.
...
기본적으로 는 이러한 검색 엔진에서 검색을 `DIL.getSearchReferrer` 인식합니다(국제 변형 포함).
...
다음 코드는 지원되는 검색 엔진에 대해 검색 레퍼러를 가져오는 방법을 보여 줍니다. 이 경우 사용자가 캐나다()에서 "homes"라는 용어를 검색했다고 [!DNL Google] `www.google.ca`가정합니다. 이 코드는 해당 검색어를 캡처하여 Audience Manager로 전송하는 데 도움이 됩니다.

**기본 코드**

검색 레퍼러( `google.com`예: 에서)를 가져오기 위한 기본 코드는 다음과 같습니다.
...
키-값 쌍에서 키에 추가된 `c_` 접두사는 신호를 고객 정의 데이터로 식별합니다. 고객 정의 데이터는 이벤트 호출 시 데이터를 전달한 특정 사이트에서 타깃팅하는 데 사용됩니다. 그러나 경우에 따라 Audience Manager 계정의 모든 속성에서 이 정보를 사용할 수 있어야 합니다. 이렇게 하려면 `c_` 키-값 쌍의 값을 플랫폼 수준 키에 매핑합니다. 플랫폼 수준 키에 접두사가 `d_` 추가되어 계정의 모든 속성을 대상으로 하는 데 신호를 사용할 수 있게 됩니다.

예를 들어 특정 사이트에서 ZIP 코드 데이터를 수집하지만 모든 Audience Manager 속성으로 타깃팅하려는 경우 플랫폼 수준에서 ZIP 코드를 사용할 수 있도록 하려면 고객이 정의한 ZIP 코드 키(예:)을 `c_zip`플랫폼 정의된 키에 연결합니다.
...
이 절차에서는 [!DNL Google Tag Manager] 계정, 해당 제품에 대한 일부 작업 지식 및 Audience Manager `dil.js` 파일을 가지고 있다고 가정합니다.

GTM에서 파일에 트래픽을 `dil.js` 추가하려면:

1. 새 컨테이너를 만들거나 기존 컨테이너를 엽니다.
1. 컨테이너에 새 태그를 추가합니다.
1. 편집할 태그를 열고 다음을 수행합니다.

   * 태그 이름을 지정합니다.
   * Select **[!UICONTROL Custom HTML Tag]** from the **[!UICONTROL Tag Type]** drop-down list.
   * HTML 필드에 스크립트 태그 내에 [!UICONTROL DIL] 코드(라이브러리 + 사용자 지정 코드)를 `<script>DIL code</script>`배치합니다.
   * 클릭 **[!UICONTROL Save]**.
