---
description: Audience Manager로 데이터를 보낼 때 따라야 하는 필수 필드, 구문, 이름 지정 규칙 및 파일 크기에 대해 설명합니다. Audience Manager/Amazon S3 디렉토리로 데이터를 전송할 때 이러한 사양에 따라 파일의 이름과 크기를 설정합니다.
seo-description: Audience Manager로 데이터를 보낼 때 따라야 하는 필수 필드, 구문, 이름 지정 규칙 및 파일 크기에 대해 설명합니다. Audience Manager/Amazon S3 디렉토리로 데이터를 전송할 때 이러한 사양에 따라 파일의 이름과 크기를 설정합니다.
...
Audience Manager로 데이터를 보낼 때 따라야 하는 필수 필드, 구문, 이름 지정 규칙 및 파일 크기에 대해 설명합니다. Audience Manager/ [!DNL Amazon S3] 디렉토리로 데이터를 전송할 때 이러한 사양에 따라 파일의 이름과 크기를 설정합니다.
...
[!DNL S3] 파일 이름에는 다음과 같은 필수 및 선택적 요소가 포함됩니다.
...
추가 예를 원하는 경우 샘플 파일을 [다운로드할](assets/ftp_dpm_1234_1445374061.overwrite) 수 있습니다. 이 파일은 `.overwrite` 파일 확장명으로 저장되었습니다. 간단한 텍스트 편집기를 사용하여 엽니다.

## 허용되는 파일 크기 {#accepted-file-sizes}

파일 처리 속도가 빠르고 빨라진 것은 물론 데이터를 [!DNL Audience Manager] / [!DNL Amazon S3] 디렉토리로 전송할 때 파일 크기 제한에 대한 아래 수치를 고려하십시오.
