...
이 문서에서는 [Privacy Service UI](https://privacyui.cloud.adobe.io/) 및 **[!DNL Privacy Service API]**&#x200B;을 통해 [!DNL Audience Manager]에게 보낼 수 있는 개별 데이터 개인 정보 및 수신 거부 요청 관리에 대한 개요를 제공합니다.

이러한 도구를 사용하면 [!DNL GDPR] 및 [!DNL CCPA]에서 수행된 소비자 데이터 개인 정보 보호 요청을 보낼 수 있습니다.
...
다음 두 가지 방법으로 [!DNL Audience Manager]에서 소비자 데이터에 액세스하고 삭제하기 위한 개별 요청을 제출할 수 있습니다.
...
개별 데이터 개인 정보 보호 요청을 전송할 때 **[Audience Manager 식별자](data-privacy-ids.md)** 섹션에 설명된 대로 해당 네임스페이스 ID(데이터 소스 ID)와 함께 모든 [!DNL Audience Manager] 식별자(ID)를 제출할 수 있습니다.
...
[Privacy Service UI](https://privacyui.cloud.adobe.io)(documentation [here](https://www.adobe.io/apis/experienceplatform/home/services/privacy-service.html#!api-specification/markdown/narrative/tutorials/privacy_service_tutorial/privacy_service_ui_tutorial.md)) 또는 Privacy Service API(documentation [here](https://www.adobe.io/apis/experienceplatform/home/services/privacy-service.html) 및 [!DNL API] 참조 [here](https://www.adobe.io/apis/experiencecloud/gdpr/api-reference.html#!acpdr/swagger-specs/privacy-service.yaml))를 통해 데이터 삭제 요청을 전송할 수 있습니다.
...
소비자 데이터 삭제 요청에 응답하여 [!DNL Audience Manager]은 요청에 포함된 [!DNL Audience Manager] 식별자와 연관된 트레이트 및 세그먼트를 삭제합니다. 또한, [!DNL Audience Manager]에 의해 추가 데이터 수집에서 옵트아웃된 개인의 해당 [!DNL Audience Manager] 식별자 및 해당 ID 매핑이 제거됩니다.

상호 장치 [!DNL CRM] ID 또는 [!DNL cookie] ID와 같은 선언된 ID를 데이터 개인 정보 보호 요청에서 전송하는 경우 [!DNL Audience Manager]는 연결된 모든 장치(선언된 ID당 최대 100개 장치)에서 필요한 삭제를 수행합니다.
...
2. 주파수가 30일 미만인 [!DNL Audience Manager]에서 업데이트를 받을 수 없습니다. 이러한 경우 [!DNL Audience Manager] 고객은 [!DNL Audience Manager]을(를) 통해 자동화된 방식으로 활성화 파트너에게 삭제 요청을 보낼 수 없습니다.

이러한 경우 [!DNL Audience Manager]을(를) 통해 자동 방식으로 정품 인증 파트너에게 삭제 요청을 보낼 수 없습니다.
...
[!DNL Audience Manager] 옵트아웃 관리와 관련하여 업계 표준 지원 [!DNL Audience Manager]에서 지원하는 옵트아웃 유형에 대한 자세한 내용은 on을 참조하십시오.

데이터 액세스 및 삭제 요청은 [Privacy Service](https://www.adobe.io/apis/experienceplatform/home/services/privacy-service.html)를 통해 처리되는 반면 옵트아웃 요청은 현재 [!DNL DCS API]를 통해 지원됩니다. 옵트아웃 [!DNL API] 호출이 어떻게 표시되어야 하는지 확인하려면 계속 읽어 보십시오.

### 글로벌 옵트아웃 요청

전역 옵트아웃은 모든 브랜드에 대한 [!DNL Audience Manager] 및 기타 [!DNL Adobe Experience Cloud] 솔루션에 걸친 옵트아웃을 나타냅니다. 아래 표에는 글로벌 옵트아웃에 사용되는 방법이 나와 있습니다.
...
파트너 수준 옵트아웃을 사용하면 특정 [!DNL Audience Manager] 파트너별로 데이터 수집에서 사용자를 옵트아웃할 수 있습니다. [!DNL CRM] ID 및 해시된 이메일 주소를 포함하여 장치 간 ID에 대한 파트너 수준 옵트아웃 요청을 보낼 수 있습니다.
...
* [!DNL Audience Manager] 는  [!DNL CRM] ID에 연결된 마지막 장치 ID 및  [!DNL CRM] ID에 대한 모든 데이터 수집, 세그멘테이션 또는 활성화를 중단합니다.
...
[!DNL Audience Manager]이(가) 파트너 수준 옵트아웃 요청을 받으면 [!DNL DCS]에서 반환되는 [!DNL JSON]에 [!DNL Audience Manager] 사용자 ID 대신 [!UICONTROL "Encountered opt out tag"] 메시지와 함께 [ 오류 코드 171](../../api/dcs-intro/dcs-api-reference/dcs-error-codes.md#opt-out-error-codes)가 포함됩니다.
...
파트너 수준 옵트아웃을 사용하면 특정 [!DNL Audience Manager] 파트너별로 데이터 수집에서 사용자를 옵트아웃할 수 있습니다. [DCS API](../../api/dcs-intro/dcs-api-reference/dcs-api-reference-overview.md)를 다음과 같이 호출하여 브랜드의 특정 기기 ID에서 데이터 수집을 옵트아웃할 수 있습니다.

| 옵트아웃에 사용하는 값 | 코드 샘플 |
|--- |--- |
| [!DNL Audience Manager] [!DNL Unique User ID](`uuid`)입니다. | `https://yourcompany.demdex.net/demoptout.jpg?d_uuid=123` |
...
소비자 데이터 개인 정보 보호 요청을 자동화하기 위해 [!DNL Audience Manager]은(는) 활성화 파트너에게 데이터 주체의 삭제 요청을 세그먼트 해제(또는 세그먼트 제거) 정보를 보내 통보합니다.

그러나 일부 활성화 파트너는

1. [!DNL Audience Manager] 및/또는
2. [!DNL Audience Manager]에서 30일 동안 두 번 이상 업데이트를 받을 수 없습니다.
...
[!DNL Audience Manager] 고객은 사용자 프로필에 대해 관련 신호/트레이트/세그먼트를 캡처하고  [오프라인 데이터 인제스트를 통해 이 정보를 전송할 수 ](../../integration/sending-audience-data/batch-data-transfer-explained/batch-data-transfer-overview.md) 있습니다 [!DNL Audience Manager]. 사용자가 동작을 반복할 경우 원래 [!UICONTROL trait] 및 [!UICONTROL segments]에 대한 자격이 계속 부여됩니다.
