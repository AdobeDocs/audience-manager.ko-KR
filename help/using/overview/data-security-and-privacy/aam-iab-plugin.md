...
Adobe는 [옵트인 기능](https://experienceleague.adobe.com/docs/id-service/using/implementation/opt-in-service/optin-overview.html?lang=ko) 및 [IAB 투명성 및 동의 프레임워크(TCF)](https://iabtechlab.com/standards/gdpr-transparency-and-consent-framework/) 지원을 통해 사용자의 개인 정보 보호 선택을 관리 및 소통할 수 있는 수단을 제공합니다.
...
IAB TCF용 Audience Manager 플러그인은 [옵트인 기능](https://experienceleague.adobe.com/docs/id-service/using/implementation/opt-in-service/iab.html?lang=ko)을 활용하며, 이 기능은 결과적으로 [ECID(Adobe Experience Platform Identity Service)](https://experienceleague.adobe.com/docs/id-service/using/home.html?lang=ko) 라이브러리의 일부입니다.
...
Audience Manager은 사용자의 개인 정보 보호 선택 사항을 준수하도록 도와주며, 사용자가 함께 일하는 모든 파트너에게 이러한 선택 사항을 쉽게 전달할 수 있는 방법을 제공합니다.
...
>
> 동의 관리에 IAB TCF용 Audience Manager 플러그인을 계속 사용하려는 고객은 [ECID](https://github.com/Adobe-Marketing-Cloud/id-service/releases) 최신 버전으로 업그레이드하여 계속 지원해야 합니다.
>
> 최신 [ECID](https://github.com/Adobe-Marketing-Cloud/id-service/releases) 버전으로 업그레이드한 후에는 IAB TCF v1.1 동의 문자열이 더 이상 지원되지 않으므로 최신 ECID 버전으로 업그레이드하기 전에 CMP를 업데이트해야 합니다.
...
>[!WARNING]
>
>IAB TCF v2.2를 지원하지 않는 CMP(동의 관리 플랫폼)를 사용하는 경우, 방문자가 유럽 연합에 있는 경우에도 Audience Manager에서 ID 동기화에서 `gdpr=0` 매개 변수를 자동으로 보냅니다. GDPR 유효성 검사가 활성 상태인지 확인하려면 IAB TCF v2.2를 지원하는지 CMP(동의 관리 플랫폼)를 확인하는 것이 좋습니다.

## 권장 사항 및 구현 방법 {#recommendations}

Audience Manager에서 IAB TCF 지원을 활성화하려면 [옵트인으로 IAB를 설정하는 방법](https://experienceleague.adobe.com/docs/id-service/using/implementation/opt-in-service/iab.html?lang=ko)에 대한 설명서를 참조하십시오.

이 작업을 수행하는 가장 쉬운 방법은 [Adobe Experience Platform 태그](https://experienceleague.adobe.com/docs/experience-platform/tags/home.html?lang=ko)를 사용하여 속성에 [!DNL ECID Opt-in]을(를) 추가하는 것입니다. [ECID 옵트인 확장](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/adobe/id-service/overview.html?lang=ko)에 대한 설명서를 읽고 Tags 확장을 설정하는 방법을 알아보십시오.

## IAB 프레임워크 사용 시 사용자 선택 워크플로우 {#user-choice-workflow}

웹 속성을 방문할 때 사용자는 게시자 및 게시자가 사용하는 서드파티 공급업체에서 데이터를 사용하는 방법에 대한 선택 사항을 제공할 수 있습니다.

사용자는 글로벌 공급업체 목록에 등록된 *타사 공급업체*&#x200B;에게 IAB 목적을 위해 *동의* 형식으로 선택 사항을 제공합니다.
...
IAB TCF v2.2에 포함된 다양한 목적 및 권한에 대한 자세한 내용은 [IAB 유럽 투명성 및 동의 프레임워크 정책](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#A_Purposes)에 나와 있습니다.
...
Audience Manager은 [IAB 유럽 투명성 및 동의 프레임워크 정책](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#Appendix_A_Purposes_and_Features_Definitions)에 정의된 다음 목적을 위해 IAB TC 문자열에 저장된 사용자의 선택 사항을 평가합니다.

* **목적 1**: 장치에 대한 정보 저장 및/또는 액세스;
* **목적 10**: 제품 개발 및 개선;
* **특수 목적 1**: 보안을 보장하고, 사기를 방지하고, 디버그를 수행하세요.

>[!IMPORTANT]
>
>Audience Manager은 쿠키를 배포하고 ID 동기화를 시작하거나 처리하려면 purpose 1, 10에 대한 동의와 공급업체 동의 가 필요합니다.
>
>[IAB 규정](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#Special_Purpose_1__Ensure_security_prevent_fraud_and_debug_)에 따라 특수 목적 1(보안 보장, 사기 방지 및 디버그)은 항상 동의하며 사용자는 이에 반대할 수 없습니다.

## Audience Manager 행동은 사용자가 동의하는지 여부에 따라 달라집니다 {#aam-behavior-consent}

Audience Manager은 IAB TC 문자열에 두 가지 목적(장치에서 정보 저장 및/또는 액세스, 제품 개발 및 개선)에 대한 사용자 동의가 포함되어 있는지 여부에 따라 다르게 작동합니다.

또한 Audience Manager에서 작업하는 모든 대상에 대해 해당 대상이 IAB TCF에 등록되어 있는 한 해당 대상에 대한 사용자 동의도 확인합니다.

| 사용자가 *동의하면* Audience Manager는 | 사용자의 동의를 *거부하면* Audience Manager는 |
|---|---|
| <ul><li>요청하신 모든 Audience Manager 사용 사례를 수행합니다.</li><li>ID 동기화 시 타사에 동의를 전달합니다(ID 동기화 호출 시 `gdpr = 1` 및 `gdpr_consent`(으)로 동의 문자열 전달).</li><li>광고 서버 픽셀에서 전달된 동의를 평가하고 적용합니다.</li><li>파트너가 시작한 ID 동기화를 수행합니다.</li></ul> | <ul><li>인스턴스에서 새 사용자 데이터를 저장하지 않습니다. 여기에는 파트너 ID, 신호, 트레이트 또는 픽셀 데이터가 포함됩니다.</li><li>타사 ID 동기화를 시작하지 않습니다.</li><li>파트너가 시작한 ID 동기화를 수행하지 않습니다.</li><li>추가 데이터 수집에서 사용자를 옵트아웃합니다.</li></ul> |

## 게시자 사용 사례 {#publisher-use-case}

IAB TCF용 Audience Manager 플러그인을 구현하면 Adobe 또는 기타 타사 공급업체의 다른 메커니즘을 통해 웹 자산에서 동의를 관리하기 위한 사용자 지정 코드를 유지 관리할 필요가 없습니다. 사용 사례는 이미지와 아래 단계에 설명되어 있습니다. 이미지의 왼쪽에서 시작하십시오.
...
3. Audience Manager이 GDPR이 적용되는지(`gdpr = 1`), 그리고 웹 자산에서 IAB TCF에 등록된 CMP가 있는지 여부를 확인합니다. 예를 들어, 유럽 연합에서 방문하는 사용자에게 적용됩니다. GDPR 플래그를 설정하는 것은 게시자로서의 책임입니다.
4. GDPR이 적용되는 경우 Audience Manager은 `gdpr_consent` 매개 변수에서 전달된 IAB TC 문자열에서 필요한 동의를 확인합니다. Audience Manager은 장치의 정보 저장 및/또는 액세스([IAB TCF 목적 1](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#A_Purposes)), 제품 개발 및 개선([IAB TCF 목적 10](https://iabeurope.eu/iab-europe-transparency-consent-framework-policies/#A_Purposes)), 데이터의 저장, 처리 또는 활성화를 위한 Audience Manager 공급업체 동의가 필요합니다.
5. IAB TC 문자열이 있고 이 문자열에 필요한 동의가 포함되어 있으면 Audience Manager은 IAB TC 문자열을 [데이터 수집 서버](../../reference/system-components/components-data-collection.md)(DCS)에 전달합니다.
6. Audience Manager이 브라우저에서 [demdex 쿠키](https://experienceleague.adobe.com/docs/core-services/interface/ec-cookies/cookies-am.html?lang=ko)를 설정하는 것으로 응답하고 타사 ID 동기화를 시작하고 수행합니다.
...
>[!IMPORTANT]
>
>IAB TCF 매개 변수가 필요한 Audience Manager 대상 파트너와 함께 작업하지만 웹 사이트에 IAB TCF를 지원하는 CMP가 없는 경우, Audience Manager에서 ID 동기화로 `gdpr=0`을(를) 보냅니다. 이는 해당 사용자에게는 GDPR이 적용되지 않음을 의미합니다.
>
...
1. 사용자는 광고 서버를 통해 광고에 노출됩니다. DCS(데이터 수집 서버)에 대한 [픽셀 호출](../../integration/media-data-integration/impression-data-pixels.md)로 변환됩니다.
2. Audience Manager가 GDPR 플래그가 적용되는지 확인합니다. 그렇지 않으면 Audience Manager은 `gdpr` 및 `gdpr_consent` 변수로 전달된 데이터를 픽셀 호출로 저장합니다.
3. IAB TC 문자열이 있고 이 문자열에 필요한 권한이 포함되어 있으면 Audience Manager은 `gdpr` 및 `gdpr_consent` 변수에서 전달된 데이터를 픽셀 호출에 저장합니다.
4. IAB TC 문자열이 없거나 필요한 권한이 부족한 경우 Audience Manager은 픽셀 호출에서 `gdpr` 및 `gdpr_consent` 변수에 전달된 데이터를 삭제합니다.
...
사이트 방문자가 적절한 권한을 제공하지 않으면 IAB TCF용 Audience Manager 플러그인은 요청에 있는 ID를 자동으로 옵트아웃합니다. 요청에 [교차 장치 ID(CRM ID)](../../reference/ids-in-aam.md)가 포함된 경우 Audience Manager은 해당 [교차 장치 ID(CRM ID)](../../reference/ids-in-aam.md)에 연결된 마지막 장치와 함께 ID를 옵트아웃합니다.

## IAB 구현 테스트 {#test-iab-implementation}

IAB TCF용 Audience Manager 플러그인을 올바르게 구현했는지 테스트하려면 [옵트인 서비스 유효성 확인의 사용 사례 4](https://experienceleague.adobe.com/docs/id-service/using/implementation/opt-in-service/testing-optin-and-iab-plugin.html?lang=ko#section-64331998954d4892960dcecd744a6d88)를 읽어 보십시오.
...
* [Adobe Experience Platform ID 서비스 옵트인](https://experienceleague.adobe.com/docs/id-service/using/implementation/opt-in-service/optin-overview.html?lang=ko)
