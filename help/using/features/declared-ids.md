...
작동 방식, 설정 절차, 코드 예제 및 변수. [!UICONTROL declared IDs]

## [!UICONTROL Declared ID] 타겟 지정 {#declared-id-targeting}

타사 ID와 같은 지속적인 스토리지 메커니즘을 사용하거나 승인하지 않는 디바이스 또는 브라우저 [!DNL Audience Manager] 의 사용자 ID를 교환하고 동기화합니다 [!DNL cookies].

## 타깃팅 [!UICONTROL Declared ID] 목적 {#declared-id-targeting-purpose}

일부 브라우저 및 대부분의 모바일 장치에서는 타사 브라우저가 허용되지 않습니다 [!DNL cookies]. 따라서 사이트 방문자에 대한 정보를 유지하거나 영구 ID를 할당하기가 어렵습니다. 이 문제를 해결하려면 이벤트 호출 [!DNL Audience Manager] 에 [!UICONTROL DIL] 합격할 수 있도록 [!UICONTROL declared IDs] 사용합니다. 또한, a는 의 모든 솔루션에서 동일한 사용자에게 적용되는 범용 ID로 작동할 [!UICONTROL declared ID] 수 [!DNL Experience Cloud]있습니다. 다음 표에서는 ID 타깃팅/일치 프로세스에 대해 설명합니다.
...
<table id="table_5D59CD5AF70B44C3B45D279283D4691F"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>작업을 수행하려면 페이지에 <span class="wintitle"> DIL </span> 와 <a href="https://docs.adobe.com/content/help/ko-KR/id-service/using/home.html" format="https" scope="external"> Adobe Experience Platform ID 서비스 </a> 코드가 있어야 합니다. <span class="wintitle"> DIL </span> 는 Adobe Experience Platform 아이덴티티 서비스에서 제공하는 <span class="wintitle"> 기능에서 선언된 ID를 </span> 받고 이 ID를 <code> setVisitorID </code> <span class="keyword"> </span> <span class="keyword"> </span>Audience Manager으로 전달합니다. </p> </td> 
  </tr> 
  <tr> 
...
시작하려면 데이터 수집에 사용할 사이트 [!DNL Experience Cloud] [!UICONTROL DIL] 의 페이지와 ID 서비스를 구성해야 합니다. DIL [만들기](../dil/dil-class-overview/dil-create.md#dil-create) 및 [선언된 ID 변수를 참조하십시오](../features/declared-ids.md#declared-id-variables).

## 옵트아웃 통화 {#opt-out-calls}

이 [!UICONTROL declared ID] 프로세스는 웹 사이트별 타깃팅 [!DNL Audience Manager] 을 옵트아웃하기 위해 사이트 방문자 기본 설정을 적용합니다. 옵트아웃 요청을 [!DNL Audience Manager] 받은 경우, 이 [!DNL JSON] 에서 반환되는 오류 코드 171에 사용자 ID 대신 메시지 [!DNL DCS] 가 포함된 오류 코드가 `Encountered opt out tag`[!DNL Audience Manager] 포함됩니다.

* [!DNL Audience Manager] 옵트아웃이 [!UICONTROL declared ID] 옵트아웃으로 [!DNL Audience Manager] 들어올 수 [!UICONTROL UUID] [!DNL URL]있습니다.
* 옵트아웃은 [!UICONTROL declared ID] 파트너 [!UICONTROL Profile Cache Server] ([!UICONTROL PCS])별로 저장됩니다. 사용하는 플랫폼 수준 옵트아웃은 없습니다 [!UICONTROL declared IDs]. 또한, [!DNL Audience Manager] 사용자를 가장자리에서 특정 영역에서 옵트아웃합니다(옵트아웃은 [!DNL DCS] 지역을 넘지 않음).
...
### 및 [!UICONTROL CID] [!UICONTROL CID_IC]
...
### 옵트아웃 [!UICONTROL DPID], [!UICONTROL DPUUID]및 [!UICONTROL UUID] (더 이상 사용되지 않음)
...
<table id="table_0E180EBE84624F338494F49D9F6EBC8E"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> <code> https://demoptout.jpg?d_dpuuid= user ID&amp;d_dpid= data provider ID </code> </p> <p>파트너 수준 옵트아웃은 이 + 쌍의 최신 매핑을 위해 <code> dpid </code> AAM UUID에 <code> dpuuid </code> 저장됩니다. 이전에 기존 매핑이 없는 경우 Audience Manager은 요청에 쿠키에 AAM UUID가 포함되어 있는지 확인하고 있다면 이를 사용하여 옵트아웃을 저장합니다. 그렇지 않은 경우 Audience Manager은 새 AAM UUID를 생성하고 그 아래에 옵트아웃을 저장합니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> d_dpuuid </code> + <code> d_dpid </code> 및 명시적 <code> d_uuid </code> </p> </td> 
   <td colname="col2"> <p> <code> https://<i>domain</i>/demoptout.jpg?d_uuid= user ID&amp;d_dpuuid= data provider's user ID&amp;<i>d_dpid=data provider ID</i> </code> </p> <p> <code> d_uuid </code> 항상 우선합니다. + <code> dpid </code><code> dpuuid </code> 조합이 다른 AAM UUID에 매핑되면 옵트아웃은 요청에서 전달된 AAM UUID 아래에 저장됩니다( <code> d_uuid </code>). </p> </td> 
  </tr> 
 </tbody> 
</table>
...
전달에 사용되는 구성 변수 [!UICONTROL declared IDs] 를 [!UICONTROL DIL] 설명합니다. [!DNL Audience Manager.]

## [!UICONTROL DIL] 에서 [!DNL Adobe Experience Platform Identity Service] 로 [!UICONTROL Declared IDs] {#dil-id-service-pass-declared-ids}

더 이상 [Adobe Experience Platform ID 서비스](https://docs.adobe.com/content/help/ko-KR/id-service/using/home.html)와 함께 사용할 때 가치 [!UICONTROL declared IDs] 가 떨어진 `dpid` 변수와 함께 전달하지 않아도 `dpuuid` 됩니다. 대신, 현재 버전의 Flash Player는 [!UICONTROL DIL] 함수 `visitorService` 에서 Flash Player [!UICONTROL declared IDs] 를 가져오는 기능을 `setCustomerIDs` [!UICONTROL Adobe Experience Platform Identity Service]사용합니다. For more information, see [Customer IDs and Authentication States](https://docs.adobe.com/content/help/en/id-service/using/reference/authenticated-state.html). 아래 `visitorService` 와 `DIL.create` 같이 전화하셨을 것입니다.
...
[!DNL Audience Manager] 시스템의 해당 사용자 ID `DPID` 및 `DPUUID` 를 조합하여 일치시킵니다. ID가 없는 경우 새 사용자 ID를 [!DNL Audience Manager] 만들고 이 ID를 조합에 `DPID/DPUUID` 동기화합니다. 사용자 ID와 일치하거나 사용자 ID를 만들면( [!DNL Audience Manager] ) 클라이언트의 도메인(퍼스트 파티) 또는 기타 로컬 저장소 `UUID`에 있는 [!DNL JSON] 응답에서 해당 ID를 반환합니다 [!DNL cookie] [!DNL cookie].
