...
이 방법론은 종종 &quot;독창적인 것을 픽셀링&quot;이라고 한다. 이러한 데이터 포인트는 캠페인의 키 보고 특성을 기반으로 모든 노출 횟수와 클릭 수를 매핑하고 보고하는 데 사용되는 타사 광고 서버 매크로가 [!DNL Audience Manager] 픽셀 코드에 동적으로 삽입됩니다. 집계된 데이터는 캠페인 성능에 대한 통합 뷰를 제공하고 사용자 지정 전환 경로를 식별하며 고객이 전환으로 이어지는 광고 서버 이벤트의 시퀀스를 개선할 수 있도록 해줍니다.
...
키-값 쌍에서 값 변수는 광고 서버가 삽입한 ID 또는 매크로입니다. 광고 태그가 로드되면 `%macro%`이(가) 필요한 해당 값으로 대체됩니다. 이 호출은 응답을 반환하지 않습니다.

## 지원되는 키-값 쌍 {#supported-key-value-pairs}

노출 이벤트 호출은 키-값 쌍으로 구성된 데이터를 허용합니다. 다음 표에서는 이러한 변수를 보유하는 데 사용되는 키를 나열하고 설명합니다. [Audience Optimization 보고서](../../reporting/audience-optimization-reports/audience-optimization-reports.md)에서 데이터를 캡처하고 분석하려는 경우 이러한 중 다수가 필요합니다.
...
<table id="table_F068C4D49F7D4775924D3CA712BF15BA"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>광고주의 데이터 소스 ID 또는 통합 코드 </p> <p><span class="wintitle"> Audience Optimization </span> 보고서에 필요합니다. </p> <p>선택 사항입니다.</p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>이 컨텍스트에서 <code> d_cid </code>은(는) 모바일 장치 유형을 고유한 사용자 ID(DPUUID)에 연결할 수 있는 키-값 쌍을 인스턴스화합니다. 고정 ID가 모바일 장치 유형을 결정합니다. 사용자 ID인 값은 다를 수 있습니다. 키-값 쌍은 인쇄되지 않는 컨트롤 문자인 <code> %01 </code>과 구분합니다. 이 매개 변수는 다음 키를 사용합니다. </p> 
...
픽셀 호출을 사용하여 [Audience Optimization 보고서](/help/using/reporting/audience-optimization-reports/audience-optimization-reports.md)에 전원을 공급할 수 있습니다. 픽셀을 사용하여 보고서의 성능을 높이려면 [메타데이터 파일에 대한 개요 및 매핑](/help/using/reporting/audience-optimization-reports/metadata-files-intro/metadata-file-overview.md)을 참조하십시오.
