...
>[!NOTE]
>
>관리자 권한이 있는 사용자만 Audience Manager 사용자 인터페이스에서 이 보고서를 볼 수 있습니다. 관리자가 아닌 사용자에게 보고서에 전자 메일을 추가하여 업로드된 인바운드 파일의 상태를 통지하도록 할 수 있습니다. 다음을 참조하십시오 [이메일 알림 수신](/help/using/reporting/onboarding-status-report.md#receive-email-notifications).
...
오류 보고 및 오류 샘플링은 [!UICONTROL Onboarding Status] 보고서.
...
<table id="table_4706D891D4C545E8BF9D8A0CC052CC48"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>오류 샘플링은 데이터 파일의 내용을 구문 분석하고 각 오류 유형에 대해 가장 일반적인 10개의 오류를 반환합니다. 인바운드 데이터 파일의 오류로 인해 개별 레코드가 처리되지 않습니다. 이 보고서를 문제 해결 도구로 사용하여 파일 오류 수를 줄이고 처리 속도를 개선하십시오. </p> <p>오류 샘플링을 수동으로 활성화해야 합니다. 활성화한 날로부터 14일 동안 실행된 다음 자동으로 꺼집니다. 14일 간격이 만료된 후 오류 샘플링을 다시 켤 수 있습니다. 오류 샘플링을 활성화하면 <a href="../features/manage-datasources.md#create-data-source"> 인바운드 데이터 소스 만들기</a> 또는 를 확인하여 <b><span class="uicontrol"> 오류 샘플링</span></b> 에서 확인란 <span class="wintitle"> 데이터 소스 설정</span> 기존 인바운드 데이터 소스의 섹션입니다. </p> <p>오류 샘플링은 계산적으로 어려운 프로세스입니다. 그 결과 각 오류 범주에 대해 처음 10개의 오류만 반환됩니다. 인바운드 데이터 소스에 포함된 모든 오류를 반환하도록 설계되지 않았습니다. 이러한 오류는 잠재적으로 크기가 더 큰 유사한 오류 집단의 대표적인 표본이다. 이 보고서에서 플래그로 표시하는 오류 유형에 대해 전체 파일을 검토하고 파일 형식을 다시 지정한 후 다시 전송하십시오. </p> <p>다음을 참조하십시오 <a href="../integration/sending-audience-data/batch-data-transfer-explained/inbound-file-contents.md"> 인바운드 데이터 파일 내용: 구문, 변수 및 예</a> 인바운드 데이터 소스에 대해 데이터 파일의 형식을 올바르게 지정하는 방법에 대한 자세한 내용 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
클릭 **[!UICONTROL Totals & Percentages]** 파일이 성공적으로 처리된 비율(%)을 확인합니다.
...
>[!NOTE]
>
>이 보고서는 현재 릴리스의 레코드 오류를 강조 표시하지 않습니다. 파일 오류를 찾아 수정하려면 결과를 검토하고 해당 오류를 [인바운드 데이터 파일 내용](../integration/sending-audience-data/batch-data-transfer-explained/inbound-file-contents.md) 설명서를 참조하십시오.
...
A [!UICONTROL Sample Error Report] 데이터 원본의 숫자 레코드가 정상적으로 처리되었는지 및 실패한 횟수를 반환합니다. 다음 단계에 따라 [!UICONTROL Sample Error Report].
...
1. 다음으로 이동 **[!UICONTROL Analytics > Onboarding Status Report]**. 데이터 소스를 검색하거나 목록에서 하나를 선택합니다.
...
<table id="table_1D44A2E6B4C847848B818190DD336841"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>다음 파일을 나열합니다. <span class="keyword"> Audience Manager</span> 귀하가 선택한 인바운드 데이터 소스에서 수신 및 처리되었습니다. </p> <p>파일 이름의 형식이 올바르지 않으면 파일 처리가 실패합니다. 파일 이름 요구 사항은 이 데이터를 로 보내는 방법에 따라 다릅니다. <span class="keyword"> Audience Manager</span>. 게재 방법은 다음과 같습니다 <span class="keyword"> Amazon</span> 및 FTP입니다. 파일 이름을 지정하는 방법에 대한 지침은 다음을 참조하십시오. </p> <p> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>구문 또는 서식 요구 사항과 일치하지 않아 처리에 실패한 레코드 수를 나열합니다. 다음을 참조하십시오 <a href="../integration/sending-audience-data/batch-data-transfer-explained/inbound-file-contents.md"> 인바운드 데이터 파일 내용: 구문, 변수 및 예</a> 데이터 형식을 지정하는 방법에 대한 자세한 정보. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b>잘못된 AAM ID</b> </p> </td> 
   <td colname="col2"> <p>형식이 잘못된 수를 나열합니다. <span class="keyword"> Audience Manager</span> 사용자 ID(UUID). 일반적으로 이는 ID를 나타냅니다. </p> 
...
<tr> 
   ...
   <td colname="col2"> <p>형식이 잘못된 전역 장치 ID의 수를 나열합니다. 다음을 참조하십시오 <a href="../reference/ids-in-aam.md">Audience Manager 내 ID 색인</a> 및 <a href="../features/global-data-sources.md">글로벌 데이터 소스</a>  장치 ID의 형식을 지정하는 방법과 장치 유형에 따라 사용해야 하는 글로벌 데이터 소스에 대한 자세한 내용은 를 참조하십시오.</p>
...
<tr> 
   ...
   <td colname="col2"> <p>온보딩된 ID입니다 <span class="keyword"> Audience Manager</span> 은(는) 기존 ID와 일치할 수 없습니다. 다음과 같은 경우 온보딩된 ID가 이 상태를 가질 수 있습니다. <span class="keyword"> Audience Manager</span> 은(는) 아직 ID 동기화를 수행하지 않았거나 동기화 후에도 ID와 일치할 수 없습니다. </p> <p>일치하지 않는 모바일 ID의 경우, <span class="keyword"> Audience Manager</span> 이(가) 다음을 수행합니다. </p> 
    <ul id="ul_B0D6AF9EB27D4017B35E36824B403879"> 
     <li id="li_D141000A50D3463182CBA4571DCC5373">이 ID를 계속 저장하고 동기화하십시오. </li> 
     <li id="li_2EFCEE716F254ABCBC5FBF749B7564E6">다음으로 기록 <span class="wintitle"> 저장된 레코드</span> ID를 동기화할 수 없는 경우 보고서에서. </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>다음과 같은 트레이트 나열 <span class="keyword"> Audience Manager</span> 은(는) 온보딩된 트레이트와 일치할 수 없습니다. 이는 다음 결과로 인해 발생할 수 있습니다. </p> 
    <ul id="ul_43619035AB6641B6949302FB50BDB5B1"> 
     <li id="li_D4C6306BF2B143198108702B309CE8CF">인바운드 데이터 파일에서 트레이트의 형식이 잘못되었습니다. 데이터 파일의 형식을 지정하는 방법에 대한 자세한 내용은 <a href="../integration/sending-audience-data/batch-data-transfer-explained/inbound-file-contents.md"> 인바운드 데이터 파일 내용: 구문, 변수 및 예</a>. </li> 
     <li id="li_A1C708A007D24EE09B7C629AFC6E43C3">에서 아직 정의되지 않은 트레이트 <span class="keyword"> Audience Manager</span>. </li> 
    </ul> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> <p> <b>기록 수신됨</b> </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>성공적으로 저장된 레코드 수입니다. 파일 형식 오류 때문에 받은 레코드 중 일부가에 저장되지 않을 수 있습니다. <span class="keyword"> Audience Manager</span>. 저장된 레코드 수는 받은 레코드 수보다 작을 수 있습니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b>실현된 총 트레이트</b> </p> </td> 
   <td colname="col2"> <p>에 저장되는 모든 인바운드 파일의 모든 사용자에 대한 트레이트 수입니다. <span class="keyword"> Audience Manager</span> 플랫폼. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b>사용되지 않은 총 신호</b> </p> </td> 
   <td colname="col2"> <p>보고서에서 수신한 총 미사용 신호 수. 이 합계는 성공적으로 저장된 총 레코드 수를 기반으로 합니다. </p> <p>다음을 참조하십시오 <a href="../reporting/dynamic-reports/unused-signals.md"> 사용되지 않은 신호 보고서</a> 추가 정보. </p> </td> 
  </tr> 
 </tbody> 
</table>
