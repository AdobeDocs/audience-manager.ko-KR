...
1. [기본 정보](../../features/segments/segment-builder.md#segment-builder-controls-basics) 섹션에서:
   ...
   * [!UICONTROL data source] 선택. 첫 번째 드롭다운 메뉴를 사용하여 Audience Manager [!UICONTROL data sources], Adobe Analytics 보고서 세트 또는 둘 다 간에 필터링합니다. 그런 다음 두 번째 드롭다운 메뉴를 사용하여 [!UICONTROL data source]을(를) 선택합니다. Adobe Analytics 보고서 세트를 사용하지 않는 경우 [!UICONTROL data source] 유형 선택기가 비활성화되고 기본적으로 Audience Manager 데이터 원본만 사용됩니다.
...
1. [트레이트](../../features/segments/segment-builder.md#segment-builder-controls-traits) 섹션에서:
   ...
   * 시계 아이콘 위로 마우스를 가져가면 [!UICONTROL trait]에 [최신성 및 빈도](../../features/segments/recency-and-frequency.md) 규칙을 추가할 수 있습니다.
   * [!UICONTROL traits]을(를) 추가하거나 제거할 때 세그먼트 모집단 데이터를 봅니다. 예상 모집단 수를 보거나 새로 고치려면 **[!UICONTROL Calculate Estimates]**&#x200B;을(를) 클릭합니다. [!UICONTROL Segment Builder]의 [세그먼트 모집단 데이터](../../features/segments/segment-builder-data.md#segment-populations)에 대해 자세히 알아보세요.
   * 완료되면 **[!UICONTROL Save]**&#x200B;을(를) 클릭합니다.

1. *(선택 사항)* [대상 매핑](../../features/segments/segment-builder.md#segment-builder-controls-destinations) 섹션에서 [!UICONTROL segment]을(를) [!UICONTROL destination]에 매핑합니다.
...
| **[!UICONTROL Data Source]** | 세그먼트를 특정 데이터 공급자와 연결합니다. <br> 첫 번째 드롭다운 메뉴를 사용하여 Audience Manager 데이터 원본, Adobe Analytics 보고서 세트 또는 둘 다 간에 필터링합니다. 그런 다음 두 번째 드롭다운 메뉴를 사용하여 데이터 소스를 선택합니다. <br> Adobe Analytics 보고서 세트를 사용하지 않는 경우 데이터 소스 유형 선택기가 비활성화되고 기본적으로 Audience Manager 데이터 소스로만 설정됩니다. |
...
## [!UICONTROL Segment]에서 [!UICONTROL Traits] 제거 {#remove-traits}

[!UICONTROL segments]에서 [!UICONTROL traits]을(를) 관리하는 것은 [!UICONTROL segments]을(를) 실행 가능하게 유지하는 데 중요한 부분입니다. [!UICONTROL segment]에서 [!UICONTROL traits]을(를) 제거해야 하는 경우 다음 단계를 따르십시오.
...
4. 삭제할 [!UICONTROL trait]을(를) 마우스로 가리킨 다음 X를 클릭합니다. 이 작업은 즉시 [!UICONTROL segment]에서 [!UICONTROL trait]을(를) 제거합니다.
...
[!UICONTROL Segment Builder]에서 [!UICONTROL destination]을(를) 선택하면 [!UICONTROL Add Destination] 대화 상자가 나타납니다. 이 창에는 [!UICONTROL destination] 유형에 따라 달라지는 [!UICONTROL destination] 및 필드에 대한 정적 정보가 표시됩니다. [!UICONTROL destination mapping]을(를) 설정하는 데 필요한 정보를 빈 필드에 제공하십시오.
