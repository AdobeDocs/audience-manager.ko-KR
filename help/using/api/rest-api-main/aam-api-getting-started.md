...
[!DNL Audience Manager] [!DNL API]s를 사용하여 작업할 때 수행해야 하는 사항입니다.
...
* **요청 헤더**:adobe  [I/](https://www.adobe.io/) O 토큰을 사용할 때는  `x-api-key` 헤더를 제공해야 합니다. [서비스 계정 통합](https://www.adobe.io/authentication/auth-methods.html#!AdobeDocs/adobeio-auth/master/AuthenticationOverview/ServiceAccountIntegration.md) 페이지의 지침에 따라 [!DNL API] 키를 가져올 수 있습니다.
...
[!DNL Audience Manager] [!DNL REST APIs]는 두 가지 인증 방법을 지원합니다.

* [JWT(서비스 계정) ](#jwt) Adobe I/O [ ](https://www.adobe.io/)를 사용한 인증. [!DNL Adobe I/O] 는 Adobe 개발자 에코시스템 및 커뮤니티입니다. 여기에는 모든 Adobe 제품에 대한 [Adobe I/O 개발자 도구 및 API](https://www.adobe.io/apis/experienceplatform.html) 및 [API가 포함됩니다](https://www.adobe.io/apis.html). 이 방법은 [!DNL Adobe] [!DNL APIs]을(를) 설정하고 사용하는 데 권장되는 방법입니다.
...
[!DNL Adobe I/O] 는 Adobe 개발자 에코시스템 및 커뮤니티입니다. 여기에는 모든 Adobe 제품에 대한 [Adobe I/O 개발자 도구 및 API](https://www.adobe.io/apis/experienceplatform.html) 및 [API가 포함됩니다](https://www.adobe.io/apis.html).

이 방법은 [!DNL Adobe] [!DNL APIs]을(를) 설정하고 사용하는 데 권장되는 방법입니다.

### 사전 요구 사항 {#prerequisites}

[!DNL JWT] 인증을 구성하려면 먼저 [Adobe I/O](https://www.adobe.io/)의 [Adobe 개발자 콘솔](https://console.adobe.io/)에 액세스할 수 있어야 합니다. 액세스 요청은 조직 관리자에게 문의하십시오.
...
1. [서비스 계정 연결](https://www.adobe.io/authentication/auth-methods.html#!AdobeDocs/adobeio-auth/master/AuthenticationOverview/ServiceAccountIntegration.md)의 단계를 따릅니다.
   * [단계 2:서비스 계정 인증](https://www.adobe.io/authentication/auth-methods.html#!AdobeDocs/adobeio-auth/master/AuthenticationOverview/ServiceAccountIntegration.md)을(를) 사용하여 프로젝트에 API를 추가하려면 [!DNL Audience Manager] [!DNL API] 옵션을 선택합니다.
1. [단계 3](https://www.adobe.io/authentication/auth-methods.html#!AdobeDocs/adobeio-auth/master/AuthenticationOverview/ServiceAccountIntegration.md)의 지침에 따라 첫 번째 [!DNL API] 호출을 수행하여 연결을 시도해 보십시오.
...
[!DNL Audience Manager] [!UICONTROL REST API]은 토큰 인증 및 갱신에 대한 [!DNL OAuth 2.0] 표준을 따릅니다. 아래 섹션에서는 인증 및 [!DNL API]s로 작업을 시작하는 방법을 설명합니다.

### 일반 [!DNL API] 사용자 {#requirements} 만들기

[!DNL Audience Manager] [!DNL API]s로 작업할 수 있도록 별도의 기술 사용자 계정을 만드는 것이 좋습니다.조직의 특정 사용자에게 연결되어 있거나 연결되어 있지 않은 일반 계정입니다. 이 유형의 [!DNL API] 사용자 계정은 다음 두 가지 작업을 수행하는 데 도움이 됩니다.

* [!DNL API]을(를) 호출하는 서비스를 식별합니다(예: [!DNL API]s를 사용하는 앱이나 [!DNL API] 요청을 만드는 다른 도구의 호출).
* [!DNL API]s에 대한 중단 없는 액세스를 제공합니다.특정 사용자에 연결된 계정은 회사를 떠날 때 삭제될 수 있습니다. 이렇게 하면 사용 가능한 [!DNL API] 코드로 작업할 수 없습니다. 특정 직원과 연결되지 않은 일반 계정은 이 문제를 해결하는 데 도움이 됩니다.

이 유형의 계정에 대한 예제 또는 사용 사례로서 [벌크 관리 도구](../../reference/bulk-management-tools/bulk-management-intro.md)를 사용하여 한 번에 많은 세그먼트를 변경한다고 가정합니다. 이렇게 하려면 사용자 계정에 [!DNL API] 액세스 권한이 필요합니다. 특정 사용자에게 권한을 추가하는 대신 해당 자격 증명, 키 및 암호를 가진 특정하지 않은 [!DNL API] 사용자 계정을 만들어 [!DNL API] 호출을 만듭니다. 이 기능은 [!DNL Audience Manager] [!DNL API]s를 사용하는 응용 프로그램을 개발할 때도 유용합니다.
...
파트너 솔루션 관리자에게 문의하십시오. 그러면 [!DNL API] 클라이언트 ID와 암호가 제공됩니다. ID와 암호는 [!DNL API]에 대해 인증합니다.

참고:새로 고침 토큰을 받으려면 [!DNL API] 액세스를 요청할 때 지정합니다.

#### 2단계:토큰 요청

원하는 [!DNL JSON] 클라이언트와 함께 토큰 요청을 전달합니다. 요청을 빌드할 때:

* `POST` 메서드를 사용하여 `https://api.demdex.com/oauth/token`을(를) 호출합니다.
* 클라이언트 ID와 암호를 기본 64로 인코딩된 문자열로 변환합니다. 전환 프로세스 동안 ID와 암호를 콜론으로 구분합니다. 예를 들어 자격 증명 `testId : testSecret`은 `dGVzdElkOnRlc3RTZWNyZXQ=`으로 변환됩니다.
...
토큰을 새로 고치면 원래 토큰이 만료된 후 [!DNL API] 액세스가 갱신됩니다. 요청 시 암호 워크플로우의 응답 [!DNL JSON]에 새로 고침 토큰이 포함됩니다. 새로 고침 토큰을 받지 않은 경우 암호 인증 프로세스를 통해 새 토큰을 만드십시오.

기존 액세스 토큰이 만료되기 전에 새로 고침 토큰을 사용하여 새 토큰을 생성할 수도 있습니다.

액세스 토큰이 만료되면 응답에 `401 Status Code`과 다음 헤더를 받게 됩니다.
...
원하는 [!DNL JSON] 클라이언트와 함께 새로 고침 토큰 요청을 전달합니다. 요청을 빌드할 때:
...
[!DNL Audience Manager] [!UICONTROL REST API]는 인증 코드와 암묵적 인증을 지원합니다. 이러한 액세스 방법을 사용하려면 사용자가 `https://api.demdex.com/oauth/authorize`에 로그인하여 토큰을 액세스하고 새로 고쳐야 합니다.
...
개체에 대해 *모든* 속성을 반환하는 [!DNL API] 메서드와 함께 이러한 선택적 매개 변수를 사용할 수 있습니다. 요청 문자열에서 해당 쿼리를 [!DNL API]에 전달할 때 이 옵션을 설정합니다.
...
| `folderId` | 지정된 폴더 내에서 [!UICONTROL traits]에 대한 모든 ID를 반환합니다. 일부 메서드에는 사용할 수 없습니다. |
| `permissions` | 지정된 권한을 기반으로 세그먼트 목록을 반환합니다. `READ` 기본값은 기본값입니다. 권한은 다음과 같습니다.<ul><li>`READ` :세그먼트에 대한 정보를 반환하고 봅니다.</li><li>`WRITE` :세그먼트  `PUT`  를 업데이트하는 데 사용합니다.</li><li>`CREATE` :세그먼트  `POST`  를 만드는 데 사용합니다.</li><li>`DELETE` : 세그먼트 삭제. 기본 트레이트에 액세스해야 합니다(있는 경우). 예를 들어 세그먼트를 제거하려면 세그먼트에 속하는 트레이트를 삭제할 수 있는 권한이 필요합니다.</li></ul><br>별도의 키-값 쌍으로 여러 권한을 지정합니다. 예를 들어 `READ` 및 `WRITE` 권한만 있는 세그먼트 목록을 반환하려면 `"permissions":"READ"`, `"permissions":"WRITE"`을 전달합니다. |
| `includePermissions` | ([!DNL Boolean]) 세그먼트에 대한 권한을 반환하려면 `true`로 설정합니다. 기본값은 `false`입니다. |

### 페이지 옵션에 대한 참고 사항

페이지 정보 *이(가)*&#x200B;가 지정되지 않은 경우 요청은 일반 [!DNL JSON]을 반환하므로 배열이 됩니다. 페이지 정보 *이(가)*&#x200B;인 경우, 반환된 목록이 총 결과 및 현재 페이지에 대한 정보를 포함하는 [!DNL JSON] 개체로 래핑됩니다. 페이지 옵션을 사용한 샘플 요청은 다음과 비슷합니다.
...
다음 표는 [!DNL API] 요청을 전달하는 데 사용되는 요청 [!DNL URLs]을 메서드별로 나열합니다.

사용하는 인증 방법에 따라 아래 표에 따라 요청 [!DNL URLs]을(를) 조정해야 합니다.

### [!DNL JWT] 인증 {#request-urls-jwt}에 대한 [!DNL URLs] 요청
...
### [!DNL OAuth] 인증에 대한 [!DNL URLs] 요청(더 이상 사용되지 않음) {#request-urls-oauth}
...
[!DNL Audience Manager] [!DNL API]s는 다른 작업 환경에 대한 액세스를 제공합니다. 이러한 환경을 사용하면 라이브 프로덕션 데이터에 영향을 주지 않고 별도의 데이터베이스에 대해 코드를 테스트할 수 있습니다. 다음 표에는 사용 가능한 [!DNL API] 환경과 해당 리소스 호스트 이름이 나와 있습니다.
...
>[!NOTE]
>
>[!DNL Audience Manager] 베타 환경은 작은 규모의 독립 실행형 프로덕션 환경입니다. 테스트할 모든 데이터는 이 환경에서 입력 및 수집되어야 합니다.

## 버전 {#versions}

이러한 [!DNL API]의 새 버전은 정기적으로 릴리스됩니다. 새 릴리스에서는 [!DNL API] 버전 번호가 증가됩니다. 버전 번호는 다음 예와 같이 요청 [!DNL URL]에서 `v<version number>`로 참조됩니다.

`https://<host>/v1/...`

## 응답 코드 정의 {#response-codes-defined}

`HTTP` 상태 코드 및 응답 텍스트가 에서 반환한  [!DNL Audience Manager] [!UICONTROL REST API]경우입니다.
