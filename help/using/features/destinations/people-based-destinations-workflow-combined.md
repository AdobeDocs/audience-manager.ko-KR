---
description: '이 페이지에는 오프라인 CRM 데이터를 Audience Manager에 이미 있는 동작 데이터와 결합하여 새 대상 세그먼트를 만든 다음 이러한 대상 세그먼트를 사용자 기반 대상으로 보내는 방법에 대한 단계별 지침이 포함되어 있습니다.  '
seo-description: '이 페이지에는 오프라인 CRM 데이터를 Audience Manager에 이미 있는 동작 데이터와 결합하여 새 대상 세그먼트를 만든 다음 이러한 대상 세그먼트를 사용자 기반 대상으로 보내는 방법에 대한 단계별 지침이 포함되어 있습니다.   '
...
이 페이지에는 오프라인 [!DNL CRM] 데이터를 이미 Audience Manager에 있는 동작 데이터와 결합하여 새 대상 세그먼트를 만든 다음 이러한 대상 세그먼트를 [!DNL People-Based Destinations]에 전송하는 방법에 대한 단계별 지침이 포함되어 있습니다.
...
>[!IMPORTANT]
>
> 이 단계는 위에 설명된 [시나리오 2](people-based-destinations-workflow-combined.md#configure-data-source-settings)에만 적용됩니다. 기존 [DPUUIDs](../../reference/ids-in-aam.md)가 이미 해시된 이메일 주소인 경우 [3단계 - 세그먼테이션에 대한 프로필 병합 규칙 만들기](people-based-destinations-workflow-combined.md#create-merge-rule)로 건너뜁니다.

기존 [DPUUIDs](../../reference/ids-in-aam.md)를 아래(오른쪽 열)의 표에서 해시된 이메일 주소에 연결하고 해시된 이메일 주소를 [1단계에서 만든 새 데이터 소스에 저장한다고 가정해 보겠습니다](people-based-destinations-workflow-combined.md#configure-data-source-settings).
...
최대 10개의 해시된 이메일 주소를 단일 [DPUUID](../../reference/ids-in-aam.md)에 연결할 수 있습니다. 이렇게 하려면 해시된 이메일 주소를 동기화 파일 내에 `<TAB>`으로 분리하십시오.

이 예제에서는 이제 두 개의 데이터 소스가 있습니다.
...
## 3단계 - 세그먼테이션을 위한 프로필 병합 규칙 만들기 {#create-merge-rule}

다음 단계는 사용자 기반 대상으로 전송할 대상 세그먼트를 만드는 데 도움이 되는 새 병합 규칙을 만드는 것입니다.
...
1. **[!UICONTROL Cross-Device Profile Options]** 목록에서 세그멘테이션을 실행할 데이터 소스를 선택합니다. 이는 기존 [DPUUIDs](../../reference/ids-in-aam.md)를 포함하는 데이터 소스여야 합니다.

## 4단계 - 대상 세그먼트 만들기 {#create-audience-segments}

새 대상 세그먼트를 만들려면 [세그먼트 빌더](../segments/segment-builder.md)를 사용하십시오. [!DNL People-Based Destinations]에 보내려는 기존 대상 세그먼트가 있는 경우 [5단계 - 사용자 기반 플랫폼 인증 구성](people-based-destinations-workflow-combined.md#configure-authentication)으로 건너뜁니다.
...
>[!IMPORTANT]
>
>Audience Manager는 특정 시간 후에 만료되는 인증 토큰을 통해 소셜 플랫폼과의 통합을 처리합니다. 만료된 토큰을 갱신하는 방법에 대한 자세한 내용은 인증 토큰 갱신 을 참조하십시오.
...
1. **[!UICONTROL Segment Mappings]** 섹션에서 이 대상에 전송할 세그먼트를 선택합니다. 이 세그먼트는 [4단계 - 대상 세그먼트 만들기](people-based-destinations-workflow-combined.md#create-audience-segments)에서 만든 세그먼트입니다.
1. 대상을 저장합니다.
