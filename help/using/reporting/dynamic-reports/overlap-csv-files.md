---
description: 중복 보고서 가 백만 개의 레코드 제한에 도달하는 경우 이 보고서에 대한 .csv 파일을 요청할 수 있습니다. "예기치 않은 오류가 발생했습니다"라는 메시지가 표시되면 보고서가 이 제한에 도달했을 수 있습니다. 자체 데이터베이스 시스템에서 가져오고 작업할 수 있는 압축된 .csv 파일을 요청하려면 고객 지원 센터에 문의하십시오. 파일은 세그먼트 간, 세그먼트 간 및 트레이트 간 중복 보고서에 사용할 수 있습니다.
...
다음 표에는 overlap .csv 파일에 사용되는 파일 이름 지정 규칙 및 파일 확장명이 나열되어 있습니다. 예에서 *기울임꼴*&#x200B;은 가변 자리 표시자를 나타냅니다.
...
<table id="table_C99FCABA365B4AB99620F27D4414E623"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p>파일 확장명 </p> </td> 
   <td colname="col2"> <p>겹치는 보고서 파일은 gzip 압축되어 있으며 파일 확장명은 <code> .gz</code>입니다. 압축을 푼 후에 <code> .csv</code> 확장을 파일에 추가해야 합니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>파일 이름 </p> </td> 
   <td colname="col2"> <p>파일 이름 구문: </p> <p> 
     <ul id="ul_D69D320A1AE94361B75D2AB47F90C4D1"> 
      <li id="li_FFB104975D104050AB67FEEC903C6E2E">세그먼트-세그먼트 파일: <code>S2S_overlap_<i>partner ID</i>_<i>yyyy-mm-dd</i>_<i>date range</i></code> </li> 
      ...
      <li id="li_CCB35A2BCB714E518AB279D453740623">특성 대 특성 파일: <code>T2T_overlap_<i>partner ID</i>_<i>yyyy-mm-dd</i>_<i>date range</i></code> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col2"> <p>보고서의 날짜 범위는 다음을 포함하는 5자리 ID입니다. </p> <p> 
     <ul id="ul_7B334CDFD7DA42AC8F383BE0F8F77FB9"> 
      <li id="li_0045DED532E747C08824DCC98A9174B3"> 7일 보고서에 대한 <code> 70000</code>. </li> 
      <li id="li_3A22775F78E648BF8AC32A9E1AF1F5DE"> 30일 보고서에 대한 <code> 30000</code>. </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
...
파일에서 문자열 데이터는 큰 따옴표로 묶여 있습니다. 아래의 모의 데이터를 참조하십시오. 간결하고 화면에 맞게 잘렸습니다.
...
[세그먼트 간 중복 보고서](segment-segment-overlap-report.md)의 데이터 파일에 다음 레코드가 포함되어 있습니다.
...
<table id="table_1BDC7019DF2543069D7AE229C5E2454E"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>기준 세그먼트와 비교 중인 세그먼트의 ID입니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> segment_name1</code> </p> </td> 
   <td colname="col2"> <p>기준 세그먼트와 비교하는 세그먼트의 이름입니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> segment_id2</code> </p> </td> 
   <td colname="col2"> <p>기준선 세그먼트의 ID입니다. 기준 세그먼트는 다른 세그먼트와 비교할 세그먼트입니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> segment_name2</code> </p> </td> 
   <td colname="col2"> <p>다른 세그먼트와 비교하는 기준선 세그먼트의 이름입니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> rangeid</code> </p> </td> 
   <td colname="col2"> <p>7일 및 30일 룩백 간격에 대한 보고서를 가져올 수 있습니다. <code> rangeid</code>은(는) 아래 표시된 시간 간격에 해당합니다. </p> <p> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>보고서 처리 날짜입니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> segment_uniques1</code> </p> </td> 
   <td colname="col2"> <p>기준 세그먼트와 비교하는 세그먼트의 고유 사용자 수입니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> segment_uniques2</code> </p> </td> 
   <td colname="col2"> <p>기준 세그먼트의 고유 사용자 수입니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> overlap_uniques</code> </p> </td> 
   <td colname="col2"> <p>기준 세그먼트와 비교를 위해 선택한 다른 세그먼트 간의 고유 사용자 겹침의 총 횟수입니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> Overlap_perc</code> </p> </td> 
   <td colname="col2"> <p>기준 요소 세그먼트와 비교를 위해 선택한 다른 세그먼트 간의 고유 사용자 겹치기 비율(%)입니다. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
[세그먼트-특성 중복 보고서](segment-trait-overlap-report.md)의 데이터 파일에 다음 레코드가 포함되어 있습니다.
...
<table id="table_45270B5D01014AD99921B320D3A32DB6"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>트레이트의 고유 사용자 수입니다. UI 보고서에서 이 숫자는 히트맵 결과에서 트레이트 위로 마우스를 가져가면 팝업 창에 표시됩니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> overlap_uniques</code> </p> </td> 
   <td colname="col2"> <p>선택한 세그먼트와 트레이트 간에 공유된 고유 사용자의 수입니다. UI 보고서에서 이 숫자는 히트맵 결과에서 트레이트 위로 마우스를 가져가면 팝업 창에 표시됩니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> trait_uniques_overlap_perc</code> </p> </td> 
   <td colname="col2"> <p>트레이트와 세그먼트 간에 겹치는 고유 사용자의 %. UI 보고서에서 이 숫자는 히트맵 결과에서 트레이트 위로 마우스를 가져가면 팝업 창에 표시됩니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> segment_uniques_overlap_perc</code> </p> </td> 
   <td colname="col2"> <p>세그먼트와 트레이트 간에 겹치는 고유 사용자 비율(%) UI 보고서에서 이 숫자는 히트맵 결과에서 트레이트 위로 마우스를 가져가면 팝업 창에 표시됩니다. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
[특성 대 특성 중복 보고서](trait-trait-overlap-report.md)의 데이터 파일에 다음 레코드가 포함되어 있습니다.
...
<table id="table_603216E6AFE4439A87C91DDFF2989F53"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>기준 트레이트와 비교하는 트레이트의 ID입니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> overlap_trait_name</code> </p> </td> 
   <td colname="col2"> <p>기준 트레이트와 비교하는 트레이트의 이름입니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> base_trait_id</code> </p> </td> 
   <td colname="col2"> <p>기준선 트레이트의 ID입니다. 기준선 트레이트는 다른 트레이트와 비교하려는 트레이트입니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> base_trait_name</code> </p> </td> 
   <td colname="col2"> <p>다른 트레이트와 비교하는 기준선 트레이트의 이름입니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> dataprovider_type</code> </p> </td> 
   <td colname="col2"> <p>데이터 공급자 ID. ID에는 다음이 포함됩니다. </p> <p> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>선택한 트레이트 간에 공유된 고유 사용자의 수입니다. UI 보고서에서 이 숫자는 히트맵 결과에서 트레이트 위로 마우스를 가져가면 팝업 창에 표시됩니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> base_trait_uniques</code> </p> </td> 
   <td colname="col2"> <p>기본 트레이트의 고유 사용자 수입니다. UI 보고서에서 이 숫자는 히트맵 결과에서 트레이트 위로 마우스를 가져가면 팝업 창에 표시됩니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> overlap_uniques</code> </p> </td> 
   <td colname="col2"> <p>선택한 트레이트 간에 공유된 고유 사용자의 수입니다. UI 보고서에서 이 숫자는 히트맵 결과에서 트레이트 위로 마우스를 가져가면 팝업 창에 표시됩니다. </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>선택한 트레이트 간에 겹치는 고유 사용자의 비율입니다. UI 보고서에서 이 숫자는 히트맵 결과에서 트레이트 위로 마우스를 가져가면 팝업 창에 표시됩니다. </p> </td> 
  </tr> 
  <tr> 
