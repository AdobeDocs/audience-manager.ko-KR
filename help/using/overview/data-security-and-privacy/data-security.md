...
다음을 말해보겠습니다. `yourcompany.demdex.com` 도메인에서 (으)로 트래픽 전송 [!DNL DCS] 경유 [!DNL HTTP]. [!DNL HSTS]에서는 대신 [!DNL HTTPS]를 사용하도록 호출을 업그레이드하고 `yourcompany.demdex.com`에서 오는 모든 후속 [!DNL DCS] 호출은 [!DNL HTTP] 대신 [!DNL HTTPS]를 사용하게 됩니다.
