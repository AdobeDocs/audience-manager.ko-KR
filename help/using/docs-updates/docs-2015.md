...
<table id="table_AC8FF0DB45B546C69D89749FF0FA1CCD"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p> 데이터 공급자 </a>에 대한 <a href="../features/audience-marketplace/marketplace-data-providers/marketplace-create-manage-feeds.md#discounts"> 할인 </p> </td> 
...
<table id="table_3A725D1D52D24CEAB49B10814008F1C7"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p><code> d_dpid</code> 및 <code> d_dpuuid</code> 대신 <code> d_cid</code> 또는 <code> d_cid_ic</code>을(를) 사용하도록 코드를 업데이트합니다. DPID 및 DPUUID 변수는 계속 작동하지만 더 이상 사용되지 않는 것으로 간주해야 합니다. 여기에는 <code> d_</code> 접두사가 없는 DPID 및 DPUUID 변형이 포함됩니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> 선언된 ID </a>에 대한 <a href="../features/declared-ids.md#variables-and-syntax"> URL 변수 및 구문 </p> </td> 
...
<table id="table_A3CAA532BFE7440C93BC11DEF5594D65"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p> Audience Optimization 보고서에 대한 데이터 및 메타데이터 파일 <a href="../reporting/audience-optimization-reports/metadata-files-intro/metadata-files-intro.md">개</a> </p> </td> 
...
<table id="table_E60674ACBD254365A73AB47FB840A333"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> <span class="keyword"> Analytics</span> 고객은 기본 Marketing Profile을 사용하고 선언된 ID를 방문자 ID 서비스를 통해 <span class="keyword"> Audience Manager</span>(으)로 전달할 때 통합 코드를 설정해야 합니다. </p> <p>이 문서는 더 이상 사용되지 않으며 <a href="../features/profile-merge-rules/merge-rules-start.md"> 교차 장치 데이터 Source 만들기</a>(으)로 대체되었습니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> <p> 세그먼트 빌더 </a>의 트레이트 및 세그먼트 크기 데이터 <a href="../features/segments/segment-builder-data.md">. </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 
     <ul id="ul_D4F6BB8E192E4C1BAF1B505541A1E56E"> 
      ...
      <li id="li_85A63519222A4423B8D7A3FF90929741"><code> declaredId</code> 변수에 메모를 추가했습니다. 방문자 ID 서비스를 사용하는 경우 DIL 대신 <code> setCustomerIDs</code>(으)로 ID를 설정합니다. <a href="https://experienceleague.adobe.com/docs/id-service/using/reference/authenticated-state.html" format="https" scope="external"> 고객 ID 및 인증 상태</a>를 참조하십시오. </li> 
     </ul> </p> </td> 
  </tr> 
 </tbody> 
</table>
