...
이 페이지는 다음을 사용하는 Audience Manager 고객을 위한 것입니다. [Audience Manager 태그 확장](https://experienceleague.adobe.com/en/docs/experience-platform/tags/extensions/client/audience-manager/overview) 웹 수집 데이터를 Audience Manager 상태로 가져오기. AppMeasurement JavaScript 라이브러리를 사용하는 고객의 경우 Audience Manager을 위해 데이터 수집 라이브러리를 업데이트하는 방법에 대한 안내서를 참조하십시오 [AppMeasurement JavaScript 라이브러리에서 웹 SDK JavaScript 라이브러리로](appmeasurement-to-web-sdk.md).
...
1. 서비스 드롭다운 메뉴에서 다음을 선택합니다. **[!UICONTROL Adobe Audience Manager]**.
1. 다음을 확인합니다. **[!UICONTROL Enable XDM Flattened Fields]** 이 옵션은 선택 취소되어 있습니다.
...
1. Adobe Experience Platform 인터페이스 왼쪽 상단에서 햄버거 아이콘을 선택한 다음 을 선택합니다 **[!UICONTROL Tags]**.
1. 원하는 태그 속성을 선택합니다.
1. 태그 속성의 왼쪽 탐색에서 을 선택합니다 **[!UICONTROL Extensions]**.
1. 선택 **[!UICONTROL Catalog]** 사용 가능한 모든 확장 목록을 보려면 상단 근처에 있습니다.
1. 을(를) 검색하고 선택합니다 **[!UICONTROL Adobe Experience Platform Web SDK]** 확장을 선택한 다음 를 선택합니다 **[!UICONTROL Install]** 오른쪽이요.

   ![카탈로그](assets/catalog.png) {style="border:1px solid lightslategray"}

1. 확장 구성 설정이 나타납니다. 를 찾습니다. **[!UICONTROL Datastreams]** 을 클릭하고 사용 중인 샌드박스와 이전 단계에서 생성한 데이터 스트림을 선택합니다.
...
1. 태그 인터페이스의 왼쪽 탐색에서 을 선택합니다. **[!UICONTROL Data Elements]**.
1. 선택 **[!UICONTROL Add Data Element]**
...
+++**4. Audience Manager 확장 대신 웹 SDK 확장을 사용하도록 규칙 업데이트**
...
1. 태그 인터페이스의 왼쪽 탐색에서 을 선택합니다. **[!UICONTROL Rules]**.
1. 편집할 규칙을 선택합니다.
1. 작업 선택 **[!UICONTROL Audience Manager - Set Variables]**
1. 이 규칙 내에 설정된 모든 Audience Manager 변수를 확인합니다. 드롭다운 메뉴에 설정된 변수와 사용자 지정 코드 내에 설정된 변수를 모두 포함합니다.
1. 변경 [!UICONTROL Action Configuration] 다음 설정으로 이동하십시오.
   * **[!UICONTROL Extension]**: [!UICONTROL Adobe Experience Platform Web SDK]
   * **[!UICONTROL Action type]**: 변수 업데이트
1. 3단계에서 작성한 데이터 객체가 오른쪽의 드롭다운에서 선택되었는지 확인합니다. **[!UICONTROL Data element]** 필드.
1. Audience Manager 키-값 쌍을 Audience Manager 확장에 구성된 것과 동일한 해당 값으로 설정합니다.
1. 웹 SDK 확장을 사용하여 모든 규칙 논리가 복제되면 다음을 선택합니다. **[!UICONTROL Keep Changes]**.
1. Audience Manager 태그 확장을 사용하여 값을 설정하는 모든 작업 구성에 대해 이 단계를 반복합니다.

위의 단계는 값을 설정하는 규칙에만 적용됩니다. 다음 단계는 를 사용하는 모든 작업을 대체합니다. [!UICONTROL Action Configuration] [!UICONTROL Send Event].

1. 웹 SDK 이벤트를 보내는 규칙을 선택합니다.
1. 작업 유형 선택 **[!UICONTROL Send Event]**.
1. 변경 [!UICONTROL Action Configuration] 다음 설정으로 이동하십시오.
   * **[!UICONTROL Extension]**: [!UICONTROL Adobe Experience Platform Web SDK]
   * **[!UICONTROL Action type]**: [!UICONTROL Send event]
1. 오른쪽에서 작업 설정을 다음과 같이 변경합니다.
   * **[!UICONTROL Type]**: 사용 **[!UICONTROL Web Webpagedetails Page Views]**.
   * **[!UICONTROL Data]**: 3단계에서 만든 데이터 개체를 선택합니다.
1. 선택 **[!UICONTROL Keep Changes]**.
...
1. 태그 인터페이스의 왼쪽 탐색에서 을 선택합니다. **[!UICONTROL Publishing Flow]**.
1. 선택 **[!UICONTROL Add Library]**.
1. 이 태그 커밋에 &quot;Web SDK로 업그레이드&quot;와 같은 이름을 지정합니다.
1. 선택 **[!UICONTROL Add All Changed Resources]**.
1. 선택 **[!UICONTROL Save]**.
...
+++**6. Audience Manager 확장 비활성화**

태그 구현이 Web SDK로 완전히 마이그레이션되면 Audience Manager 확장을 비활성화할 수 있습니다.

1. 태그 인터페이스의 왼쪽 탐색에서 을 선택합니다. **[!UICONTROL Extensions]**.
1. 을(를) 찾아 선택합니다 [!UICONTROL Audience Manager] 확장명. 오른쪽에서 을 선택합니다. **[!UICONTROL Disable]**.
1. 위의 동일한 게시 작업 과정을 따라 의 제거를 게시하십시오. [!UICONTROL Audience Manager] 확장명.
1. 프로덕션에서 확장이 비활성화되면 완전히 제거할 수 있습니다. 확장을 선택하고 오른쪽에 있는 세 점 메뉴를 선택한 다음 를 선택합니다 **[!UICONTROL Uninstall]**.
