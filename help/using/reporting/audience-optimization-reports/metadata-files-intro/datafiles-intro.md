...
적절한 이름 및 형식 지정된 데이터 파일을 사용하면 노출, 클릭 또는 전환 데이터를 [Audience Optimization 보고서](../../../reporting/audience-optimization-reports/audience-optimization-reports.md)로 가져올 수 있습니다. 이 기능은 [!DNL Audience Manager]과(와) 통합되지 않고 해당 보고서 세트에서 해당 데이터를 사용하여 작업하려는 파트너와 함께 작업할 때 유용합니다. 이 프로세스에서는 노출, 클릭 및 전환 데이터를 위해 개별 파일이 필요합니다. 이러한 이벤트를 하나의 파일로 취합하지 마십시오.
...
다음 구문은 형식이 올바른 데이터 파일 이름의 구조를 정의합니다. 참고: *기울임체*&#x200B;는 파일 내용에 따라 변경되는 변수 자리 표시자를 나타냅니다.
...
* 데이터 클릭: <pre>클릭_<i>yyymdd</i>.gz</code></pre>
* 전환 데이터: <pre>conversions_<i>yyymdd</i>.gz</code></pre>

## 데이터 파일 {#content-format} 컨텐츠 형식

다음 구문은 형식이 올바른 데이터 파일의 콘텐츠 구조를 정의합니다. 참고: *기울임체*&#x200B;는 변수 자리 표시자를 나타내며 실제 데이터 파일의 레이블로 대체됩니다.
...
* *파일은 버전 번호로 종료되어야 합니다.* 현재 버전은 1.1입니다.
* 파일 헤더와 내용을 인쇄되지 않는 ASCII 001 문자로 구분합니다. ASCII 001을 사용할 수 없는 경우 헤더와 데이터를 탭 구분 기호로 구분합니다. 이러한 문자는 인쇄되지 않는 문자이므로 위의 구문 예에는 표시 목적으로만 파이프 `"|"`가 표시됩니다.
...
<table id="table_D8C5068741C3460380505F95F3016757"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>사이트 방문자 ID(데이터 공급자 고유 사용자 ID<span class="term"> 또는 DPUUID라고도 함)</span> </p> </td> 
  </tr> 
  <tr> 
...
노출, 클릭 또는 변환 데이터 파일을 [!DNL Audience Manager] 계정에 대한 Amazon S3 디렉토리로 업로드합니다. 배달/디렉토리 경로, 파일 처리 시간 및 업데이트에 대한 자세한 내용은 이 섹션을 참조하십시오.
...
**구문:** <pre>.../log_ingestion/pid= <i>AAM ID<i>/dpid= <i>d_src</i>/logs/ <i>파일 유형</i>_<i>yyymdd</i></code></pre>
...
메타데이터 파일 이름 지정 및 만들기에 대한 요구 사항을 검토하십시오. 시작하려면 [메타데이터 파일에 대한 개요 및 매핑](../../../reporting/audience-optimization-reports/metadata-files-intro/metadata-file-overview.md)을 참조하십시오.
