...
추가 및 제거 [!UICONTROL traits] 위치: [!UICONTROL Segment Builder] 실제 항목을 보려면 [!UICONTROL trait] 실제 및 예상 세그먼트 인구 데이터와 함께 인구. 예상 모집단 크기 데이터를 사용하면 캠페인에 적합한 세그먼트를 만드는 데 도움이 됩니다.

## [!UICONTROL Trait] 모집단 데이터 {#trait-population-data}

[!UICONTROL Segment Builder] 다음을 표시합니다 [!UICONTROL Total Trait Population] 을(를) 추가할 마지막 날 동안 [!UICONTROL trait] 세그먼트 로 이동합니다. 이 데이터는 선택한 항목 주위의 파란색 필드에 나타납니다 [!UICONTROL trait] 다음에서 [!UICONTROL Basic View] 섹션.
...
위치 [!UICONTROL Segment Builder], 예상 모집단 그래프의 파란색 막대는 세그먼트 크기에 대한 가능한 상위 및 하위 범위를 나타냅니다. 과거 성능이 미래 결과를 보장하지는 않지만 예상 데이터를 통해 새 세그먼트나 편집된 세그먼트의 잠재적 크기를 파악할 수 있습니다.

## 세그먼트 모집단 데이터 개요 {#segment-populations}

[!UICONTROL Segment Builder] 세그먼트를 만들고 편집할 때 세그먼트 모집단 데이터를 표시합니다.

* 예상 세그먼트 모집단 데이터(실시간 및 합계)의 경우 [!UICONTROL Segment Builder] 는 세그먼트에서 트레이트를 추가하거나 제거할 때 그래프를 자동으로 업데이트하지 않습니다. 클릭 **[!UICONTROL Calculate Estimates]** 예상 모집단 수를 보거나 새로 고칩니다.

* 실제(실제) 세그먼트 인구 데이터의 경우(실시간 및 합계), [!UICONTROL Segment Builder] 기존 세그먼트를 로드할 때 세그먼트 그래프가 자동으로 업데이트됩니다. 새 세그먼트의 경우 또는 기존 세그먼트에 새 트레이트를 추가할 때 실제 모집단 데이터는 세그먼트가 생성된 후 24시간이 지나야 업데이트됩니다.
...
<table id="table_B24503F372E34B6BBDF5204181701A59"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>지정된 시간 범위 동안 실시간으로 확인되었으며, Audience Manager에 의해 확인되었을 때 세그먼트에 대한 자격이 있는 예상 고유 방문자 수입니다. </p> <p>위치 <span class="wintitle"> 세그먼트 빌더</span>, 트레이트에 대한 마지막 30일 인구(<span class="wintitle"> 총 트레이트 인구</span>)는 실시간으로 평가되는 트레이트 및 세그먼트에 대해 다를 수 있습니다. </p> <p>
     ...
     </ul> </p> <p> <p>참고: <span class="wintitle"> 예상 실시간 모집단</span> 지표에는 가 제공한 연결을 기반으로 세그먼트에 적합한 장치가 포함되지 않습니다. <span class="wintitle"> 프로필 병합 규칙</span> 을(를) 사용하는 <a href="../../features/profile-merge-rules/merge-rule-definitions.md#device-options"> 장치 그래프 옵션</a>. </p> </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col2"> <p>새 세그먼트나 수정된 세그먼트에 있을 수 있는 예상 고유 방문자 수입니다. 거의 모든 예측과 마찬가지로 과거 성과가 미래 결과를 보장하지는 않지만 예상 합계를 사용하여 다음을 수행할 수 있습니다. </p> <p> 
     ...
     </ul> </p> <p> <p>참고: <span class="wintitle"> 예상 총 인구</span> 지표에는 가 제공한 연결을 기반으로 세그먼트에 적합한 장치가 포함되지 않습니다. <span class="wintitle"> 프로필 병합 규칙</span> 을(를) 사용하는 <a href="../../features/profile-merge-rules/merge-rule-definitions.md#device-options"> 장치 그래프 옵션</a>. </p> </p> </td> 
  </tr> 
 </tbody> 
</table>
...
[!UICONTROL Profile Merge Rules] 은 실제 실시간 및 총 인구 수에 영향을 줍니다. 이러한 합계는 다음의 경우에 따라 달라집니다. [!UICONTROL Profile Merge Rule] 세그먼트는 장치 그래프 옵션을 사용하거나에 속합니다. 다음을 참조하십시오. [정의된 프로필 병합 규칙 옵션](../../features/profile-merge-rules/merge-rule-definitions.md).

### 세그먼트 모집단 데이터 [!UICONTROL Merge Rules] 없이 [!UICONTROL Device Graph Option]

다음 표는 세그먼트를 사용할 때의 실제 실시간 및 총 인구 지표를 정의합니다. [!UICONTROL Profile Merge Rule] 이(가) 없이 만들어짐 [!UICONTROL device graph] 옵션을 선택합니다. 장치 옵션 설정입니다 **[!UICONTROL No Device Options]** 및 **[!UICONTROL Current Device Proflie]**.
...
<table id="table_A18C973855DB46A0B39B81F32E0E7540"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>지정된 시간 범위 동안 실시간으로 확인되었으며, Audience Manager에 의해 확인되는 순간 세그먼트에 대한 자격이 되는 실제 고유 방문자 수입니다. </p> <p>위치 <span class="wintitle"> 세그먼트 빌더</span>, 트레이트에 대한 마지막 30일 인구(<span class="wintitle"> 총 트레이트 인구</span>)은 실시간으로 평가되는 트레이트 및 세그먼트에 대해 다를 수 있습니다. </p> <p> 
...
### 세그먼트 모집단 데이터 [!UICONTROL Merge Rules] 포함 [!UICONTROL Device Graph] 옵션

다음 표는 세그먼트를 사용할 때의 실제 실시간 및 총 인구 지표를 정의합니다. [!UICONTROL Profile Merge Rule] 을(를) 사용하여 생성됨 [!DNL device graph] 옵션을 선택합니다. 다음은 의 장치 옵션 설정입니다. [!UICONTROL Profile Link Device Graph], [!DNL Adobe] [!DNL device graph], 및 기타 서드파티 [!DNL device graph] 사용 가능한 선택 사항입니다.
...
[!UICONTROL Segment Builder] 는 최대 4개의 최신성 및 빈도 표현식을 포함하는 세그먼트 규칙에 대한 세그먼트 크기 예측을 지원합니다. 세그먼트 규칙을 작성할 때 최신성 및 빈도 표현식을 4개 이상 선택하면 모집단을 추정할 때 세그먼트 추정기에 오류가 표시됩니다.

### 제한 사항 [!UICONTROL Merge Rules] 세그먼트 모집단 예상 시

현재 세그먼트 크기 추정기에서 다음을 고려하지 않기 때문에 알려진 제한 사항이 있습니다 [!UICONTROL profile merge rules]. 예를 들어 **[!UICONTROL No Authenticated Profile + Current Device Profile]** [병합 규칙](../../features/profile-merge-rules/merge-rule-definitions.md). 현재 세그먼트 예상 수를 계산하는 방법 때문에 예상 모집단에는 인증된 프로필이 포함됩니다. 그러나 기존 세그먼트 모집단은 인증된 프로필을 올바로 무시합니다.
