...
seo-description: 이 페이지에서는 투명도가 향상된 온라인 광고 타깃팅을 위해 웹 기반 Audience Manager 대상 세그먼트를 Facebook에 보낼 목적으로 Facebook 웹 사이트 사용자 지정 대상(WCA) 픽셀을 만드는 프로세스를 보여줍니다.
...
이 페이지에서는 투명도가 향상된 온라인 광고 타깃팅을 위해 웹 기반 [!DNL Audience Manager] 대상 세그먼트를 [!DNL Facebook]에 보낼 목적으로 [!DNL Facebook Website Custom Audiences]([!DNL WCA]) 픽셀을 만드는 프로세스를 보여줍니다.

## 개요 {#overview}

[WCA(facebook 웹 사이트 사용자 지정 대상)](https://www.facebook.com/business/help/449542958510885) 를 사용하면 특정 페이지를 방문하거나 웹 사이트에서 특정 작업을 수행한 사람 목록을 만들 수 있습니다. [!DNL Audience Manager] 대상 [!DNL WCA] 을  [!DNL URL] 사용하여에서 활성화할 수 있습니다. 이 통합을 통해 타깃팅할 웹 기반 대상을 보내도록 사용자 지정 픽셀 기반 통합 [!DNL Facebook] 을 구성할 수 있습니다.

![Facebook WCA 통합](/help/using/integration/assets/facebook-wca-integration.png)

>[!IMPORTANT]
>
> 이 기능을 사용하려면 [URL 대상](/help/using/features/destinations/create-url-destination.md)에서 소셜 플랫폼에 대한 [!UICONTROL Website] 대상 옵션을 선택해야 합니다. 소셜 플랫폼에서는 레퍼러 정보가 플랫폼으로 전송될 때 마스크가 해제되어야 합니다. 즉, 대상 플랫폼/파트너가 레퍼러 [!DNL URL]에 있는 정보를 볼 수 있습니다.
...
* **[!UICONTROL URL type]**: Select **[!UICONTROL Website audience for social platforms]**. 이 [!UICONTROL URL Type] 옵션을 선택하면 [!DNL Facebook WCA] 픽셀을 실행할 때 [!DNL Audience Manager] 레퍼러 [!DNL URL] 정보가 모호하지 않습니다.
...
| `%ALIAS%` | [!DNL Audience Manager] 매크로는 사이트 방문자가 적격한 [!DNL Audience Manager] [!UICONTROL segment] ID로, 쉼표로 구분됩니다. |

[!UICONTROL URL destination] 구성은 아래 이미지에서 볼 수 있습니다.
...
[URL 대상 구성](/help/using/features/destinations/create-url-destination.md) 워크플로우에서 해당 세그먼트를 새로 만든 [!UICONTROL destination]에 매핑합니다. 매핑 값이 [!DNL Audience Manager] [!UICONTROL segment ID]로 자동 채워지는지 확인합니다.
...
이제 [!DNL Audience Manager] 세그먼트를 [!DNL Facebook WCA] 대상에 할당했으므로 [!DNL Audience Manager]에서 픽셀의 각 세그먼트 ID로 지정된 세그먼트의 사용자에게 [!DNL Facebook WCA] 픽셀을 선택적으로 실행하여 [!DNL Facebook Audience]를 채웁니다. 따라서 [!DNL Facebook Audience]에 점차적으로 증가하면 사이트의 적용 가능한 대상에 태그가 더 많이 실행됩니다.

>[!NOTE]
>
> 사용자가 [!DNL Audience Manager] 세그먼트를 벗어나는 경우 현재 [!DNL Audience Manager]이 [!DNL Facebook]에 사용자를 [!DNL Custom Audience]에서 제거할 수 있는 방법이 없습니다.
