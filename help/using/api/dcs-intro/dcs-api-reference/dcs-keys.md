...
<table id="table_23B7E15EC13749E9A245DFB543822DB7"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>개인, 고객 정의 속성. </p> <p> 키에 <code> p_</code> 접두사가 있을 때 DCS는 사용자의 개인 데이터를 허용합니다. 개인 데이터는 트레이트 평가에 사용되지만 시스템에 기록되거나 저장되지 않습니다. 예를 들어, 트레이트가 <code> customers = p_age&lt;25</code> 정의된 후 이벤트 <code> p_age=23</code> 호출에서 전달되었다고 가정해 봅시다. 이러한 조건이라면 연령 기반 자격 조건을 충족하는 사용자가 트레이트에 가입할 수 있지만, Audience Manager가 요청을 받은 후 키-값 <span class="keyword"> 쌍이</span> 삭제되고 기록되지 않습니다. </p> </td>
  </tr> 
 </tbody> 
</table>
...
<table id="table_FCCE4F9D796648899772A191981EFDE6"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Audience Manager에서 할당한 하나 이상의 데이터 공급자 ID(<code> DPID</code>) 및 데이터 공급자 사용자 ID(<code> DPUUID</code>)를 <span class="keyword"> 포함합니다</span>. 여러 개의 <code> DPID</code>s 및 <code> DPUUID</code>s 쌍을 사용하는 경우 각 쌍을 인쇄되지 않는 문자와 <code> %01</code>구분합니다. 예: <code><i>DPID</i>%01<i>DPUUUID</i></code>. </p> <p><code> d_cid</code> 를 <code> d_dpid</code> <code> d_dpuuid</code>대체합니다. 이 값은 더 이상 사용되지 않지만 여전히 지원됩니다. <a href="../../../reference/cid.md">CID가 DPID 및 DPUUID 대체</a>를 참조하십시오. </p> </td>
  </tr>
  <tr> 
   <td colname="col1"> <p><code> d_cid_ic</code> </p> </td> 
   <td colname="col2"> <p>단일 키-값 쌍에 통합 코드와 관련 고유 사용자 ID가 들어 있습니다. </p> <p><code> d_cid_ic</code> 를 <code> d_dpid</code> <code> d_dpuuid</code>대체합니다. 이 값은 더 이상 사용되지 않지만 여전히 지원됩니다. <a href="../../../reference/cid.md">CID가 DPID 및 DPUUID 대체</a>를 참조하십시오. </p> </td>
  </tr> 
  <tr> 
   <td colname="col1"> <p><code> d_coppa</code> </p> </td> 
   <td colname="col2"> <p>하위 보호 규정을 준수하기 위해 타사 쿠키 사용을 비활성화합니다. 이 매개 변수는 Adobe Experience Platform ID 서비스에 의해 동적으로 설정되며 <code> idSyncDisable3rdPartySyncing</code> 구성에 따라 달라집니다. Adobe <a href="https://marketing.adobe.com/resources/help/en_US/mcvid/mcvid_coppa.html" format="https" scope="external"> Experience Platform Identity Service의 COPPA 지원을 참조하십시오</a>. </p> </td>
  </tr>
  <tr> 
   ...
   <td colname="col2"> <p>선택 사항입니다. 고객 요청 시 활성화됩니다. Adobe Audience Manager 컨설턴트 또는 고객 지원 센터에 문의하십시오. </p> <p>트레이트 및 세그먼트가 <code> JSON</code> 응답 내에서 반환되어야 함을 나타냅니다. </p> <p> 
     <ul id="ul_8B936ACB18724681B959783421ACF026"> 
      <li id="li_792A6248F49141C0B4B214C754D5F5C5"> <p><code> d_cts=1</code> 세그먼트에 대한 <a href="../../../reference/ids-in-aam.md"> 이전 세그먼트 ID를</a> 반환합니다. </p> </li>
  ...
  <tr> 
   ...
   <td colname="col2"> <p>삭제 예정. 를 <code> d_cid</code> 참조하십시오 <code> d_cid_ic</code>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p><code> d_dst_filter</code> 는 예약된 특성이며, Adobe Analytics와 Audience Manager 간의 통합에 사용됩니다. </p> <p>예약된 특성을 사용하는 트레이트를 만들지 않는 것이 좋습니다. Adobe는 언제든지 예약 속성을 변경할 수 있습니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p><code> d_jsonv=1|0</code> </p> </td> 
   <td colname="col2"> <p>응답에 사용할 버전을 <code> JSON</code> 나타냅니다. 일반적으로 이것을 로 설정해야 합니다 <code> d_jsonv=1</code>. 설정을 <code> d_jsonv=0</code> 사용하면 ID 동기화가 비활성화됩니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>삭제 예정. <code> d_rs</code> 는 예약된 특성이며, Adobe Analytics와 Audience Manager 간의 기존 통합에 <span class="keyword"></span> 사용됩니다 <span class="keyword"></span>. </p> <p>예약된 특성을 사용하는 트레이트를 만들지 않는 것이 좋습니다. Adobe는 언제든지 예약 속성을 변경할 수 있습니다. </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col2"> <p>DCS에서 <code> JSON</code> 응답을 받으려면 <span class="wintitle"> 필수입니다</span>. </p> <p> 
     <ul id="ul_9EA00BD822504BCA8ECB59C1634DB91A"> 
      ...
      <li id="li_824C23B4C7AA4B5EBADF73D26016A18E">이 값을 포함하면 <span class="wintitle"> DCS는</span> 응답 본문에 <code> JSON</code> 개체를 반환합니다. 아래 예를 참조하십시오. 응답이 더 복잡할 수 있습니다. </li> 
     </ul> </p> <p> 
     <code class="syntax javascript">
      {
      &nbsp;&nbsp;&nbsp;&nbsp;"stuff":&nbsp;[],
  ...
  <tr> 
   ...
   <td colname="col2"> <p><code> SID</code> 점수 ID를 <span class="term"> 나타냅니다</span>. 트레이트 또는 세그먼트에 대한 고유 ID입니다. </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col2"> <p>트레이트 평가를 위해 데이터 소스를 전달합니다. 이 데이터 소스의 트레이트만 평가됩니다. </p> <p>예를 들어 다음과 같은 기능을 사용할 수 있습니다. </p> <p> 
     ...
     </ul> </p> <p>샘플 호출에서는 <code>yourcompany.demdex.net/event?key1=val1&amp;key2=val2&amp;d_tdpid=1</code>트레이트 T1만 반환됩니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> d_tdpid_ic</code> </p> </td> 
   <td colname="col2"> <p>목적은 위에 설명된 <code> d_tdpid</code> 매개 변수와 동일합니다. 그러나 이 경우 통합 코드를 사용하여 데이터 소스가 전달됩니다. </p> <p>위에 설명된 트레이트를 유지하려면 샘플 호출을 고려하십시오. </p> <p>예를 <code>yourcompany.demdex.net/event?key1=val1&amp;key2=val2&amp;d_tdpid_ic=ic2</code>들어 특성 T2만 반환됩니다. </p> </td> 
  </tr> 
  <tr> 
