...
| [보고서 겹치기](/help/using/reporting/dynamic-reports/dynamic-reports.md) | 겹침 보고서([특성-특성](/help/using/reporting/dynamic-reports/trait-trait-overlap-report.md), [세그먼트-특성](/help/using/reporting/dynamic-reports/segment-trait-overlap-report.md), [세그먼트-세그먼트 간](/help/using/reporting/dynamic-reports/segment-segment-overlap-report.md))를 계산하는 데 사용하는 방법을 업데이트했습니다. 겹치기 보고서 계산은 이제 보고 고유 및 MinHash 서명을 기반으로 합니다(데이터 샘플링[이 아닌).](/help/using/reporting/report-sampling.md#data-sampling-ratio) |
...
| [인바운드 S3 ](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-s3-filenames.md) 파일 이름 및  [인바운드 FTP 파일 이름](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-ftp-filenames.md) | 파일 이름 구문의 `DPID` 및 `DPID_TARGET_DATA_OWNER` 필드에 대한 정의 및 제공된 예제를 업데이트했습니다. |

## 2020년 8월 {#aam-august-updates}

| 주제 | 설명 |
|---- |----|
| [사람 기반 대상에 대한 Google 고객 일치](../features/destinations/people-based-destinations-prerequisites.md#gcm) | 이제 [!UICONTROL People-Based Destinations]을(를) 사용하여 [!DNL Google Customer Match]을(를) 통해 사용자를 타깃팅할 수 있습니다. [!UICONTROL People-Based Destinations]을(를) 사용하여 자사 대상 세그먼트를 [!DNL Google Customer Match] 대상으로 보내려면 먼저 [!DNL Google]이(가) 사용자를 해당 허용 목록에 추가해야 합니다. 자세한 내용은 [사용자 기반 대상](../features/destinations/people-based-destinations-prerequisites.md#gcm) 설명서를 참조하십시오. |
| [새 SFTP 서버](../faq/faq-data-collection.md) | 이제 [!DNL S3]보다 [!DNL AWS SFTP Gateway]을(를) 사용하는 새 [!DNL SFTP] 서버([!DNL ftp-in-gtw] 및 [!DNL ftp-out-gtw])를 사용할 수 있습니다. |
...
| 주제 | 설명 |
|---- |----|
| [Audience Manager와 Adobe Experience Platform 간의 대상 공유](/help/using/integration/integration-aep/aam-aep-audience-sharing.md#aep-aam-segment-population-differences) | 새 섹션인 **Audience Manager과 Experience Platform**&#x200B;의 세그먼트 모집단 차이를 이해하면 모집단 수가 Experience Platform과 Audience Manager 간에 달라질 수 있는 가능한 이유를 설명합니다. |
| [Audience Manager REST API 시작하기](/help/using/api/rest-api-main/aam-api-getting-started.md#jwt) | [!DNL Adobe I/O]**을(를) 사용하여 새 섹션인** JWT(서비스 계정) 인증을 추가했습니다. [!DNL Adobe I/O]을(를) 사용하여 Audience Manager API에 인증하려면 이 섹션을 참조하십시오. |
