...
| 주제 | 설명 |
|----|----|
| [Predictive Audiences](/help/using/features/algorithmic-models/predictive-audiences.md) | <ul><li>**Predictive Audiences 모델 복제**: 이제 기존 모델을 복제하고 필요에 따라 구성을 변경할 수 있습니다. 읽기 [예측 대상 모델 복제 및 편집](/help/using/features/algorithmic-models/predictive-audiences.md#clone-predictive-audiences) 추가 정보.</li><li>**Predictive Audiences 일괄 분류**: 이제 Predictive Audiences는 실시간으로 개별 성향으로 방문자를 분류하는 것 외에도 배치 분류를 지원하므로 온보딩된 사용자를 분류하고 배치 대상으로 활성화할 수 있습니다.</li></ul> |
| [겹치기 보고서](/help/using/reporting/dynamic-reports/dynamic-reports.md) | 겹치기 보고서( )를 계산하는 데 사용하는 방법을 업데이트했습니다.[특성 대 특성](/help/using/reporting/dynamic-reports/trait-trait-overlap-report.md), [세그먼트-트레이트](/help/using/reporting/dynamic-reports/segment-trait-overlap-report.md), [세그먼트-세그먼트](/help/using/reporting/dynamic-reports/segment-segment-overlap-report.md)). 겹치기 보고서 계산은 이제 보고 고유 사항 및 MinHash 서명(대신)을 기반으로 합니다. [1/54 데이터 샘플링](/help/using/reporting/report-sampling.md#data-sampling-ratio)). |
...
| 주제 | 설명 |
|---- |----|
| [Audience Manager에서의 접근성](/help/using/overview/accessibility-in-aam.md) | 접근성이란 시각, 청각, 인지, 모터 또는 기타 장애와 같은 다양한 장애가 있는 사용자가 가능한 한 적은 노력으로 소프트웨어 제품을 사용할 수 있도록 하는 일련의 기능을 의미합니다. 모든 사람이 Audience Manager을 더 쉽게 사용할 수 있도록 이 문서에 설명된 여러 접근성 기능에 대한 지원을 개발했습니다. |
| [인바운드 S3 파일 이름](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-s3-filenames.md) 및 [인바운드 FTP 파일 이름](/help/using/integration/sending-audience-data/batch-data-transfer-explained/inbound-ftp-filenames.md) | 정의를 업데이트하고 다음에 대한 예를 제공했습니다. `DPID` 및 `DPID_TARGET_DATA_OWNER` 파일 이름 구문의 필드입니다. |

## 2020년 8월 {#aam-august-updates}

| 주제 | 설명 |
|---- |----|
| [사용자 기반 대상을 위한 Google Customer Match](../features/destinations/people-based-destinations-prerequisites.md#gcm) | 이제 다음을 사용할 수 있습니다. [!UICONTROL People-Based Destinations] 을 통해 사용자를 타깃팅하려면 [!DNL Google Customer Match]. 을(를) 사용하기 전에 [!UICONTROL People-Based Destinations] 자사 대상 세그먼트를 [!DNL Google Customer Match] 대상, 필수 사항 [!DNL Google] 이(가) 사용자를 해당 허용 목록에 추가합니다. 다음 확인: [사용자 기반 대상](../features/destinations/people-based-destinations-prerequisites.md#gcm) 자세한 내용은 설명서를 참조하십시오. |
| [새 SFTP 서버](../faq/faq-data-collection.md) | 이제 새 를 사용할 수 있습니다. [!DNL SFTP] 서버 ([!DNL ftp-in-gtw] 및 [!DNL ftp-out-gtw]) [!DNL AWS SFTP Gateway] 초과 [!DNL S3]. |
...
| 주제 | 설명 |
|---- |----|
| [Audience Manager와 Adobe Experience Platform 간의 대상 공유](/help/using/integration/integration-aep/aam-aep-audience-sharing.md#aep-aam-segment-population-differences) | 새 섹션, **Audience Manager과 Experience Platform 간의 세그먼트 모집단 차이점 이해**&#x200B;는 Experience Platform과 Audience Manager 간에 모집단 수가 다를 수 있는 가능한 이유를 설명합니다. |
| [Audience Manager REST API 시작](/help/using/api/rest-api-main/aam-api-getting-started.md#jwt) | 새 섹션을 추가했습니다. **를 사용하는 JWT(서비스 계정) 인증[!DNL Adobe I/O]**. 다음을 사용하여 Audience Manager API에 인증하려면 이 섹션 을 참조하십시오. [!DNL Adobe I/O]. |
