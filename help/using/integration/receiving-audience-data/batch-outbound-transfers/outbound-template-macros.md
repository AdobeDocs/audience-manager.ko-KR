...
이 표에서는 파일 이름과 헤더 필드를 정의하는 데 사용할 수 있는 매크로를 나열하고 설명합니다. 코드 샘플은 아웃바운드 [매크로 예를 참조하십시오](../../../integration/receiving-audience-data/batch-outbound-transfers/outbound-macro-examples.md).
...
<table id="table_C353AF028E0A4944A8727FD01C94FDB6"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>아웃바운드 파일을 여러 부분으로 분할함을 나타냅니다. 파일 이름의 SPLITNUM 섹션을 0으로 바꾸고 SPLITNUM 섹션의 최소 3자를 지정합니다.</p>
...
<p>위의 예에서 마지막 세 자리 숫자(001,002,003)는 SPLITNUM 식별자입니다.</p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>동기화 유형을 나타내며 다음을 포함합니다. </p> 
    <ul id="ul_CA5057DA18144AB8BC17B3EB79891B25"> 
     ...
     <li id="li_1A7BBBB40AD94FC39B06F4FC49586595"> <code> 로그 </code>:증분 동기화. </li> 
    </ul> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>구분 문자로 사용되는 이 매크로는 필드 사이에 탭을 삽입합니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> 타임스탬프 </code> </p> </td> 
   <td colname="col2"> <p>10자리, UTC, Unix 타임스탬프. </p> <p>또한 &lt;TIMESTAMP; <code> 로 형식을 지정할 수 있습니다.format="YYYYMMDDhmmss"&gt; </code> Java 날짜/타임스탬프 서식 지정 규칙 다음에 나옵니다. </p> </td> 
  </tr>
...
데이터 파일의 내용 서식을 지정하는 데 사용되는 매크로입니다. 코드 샘플은 아웃바운드 [매크로 예를 참조하십시오](../../../integration/receiving-audience-data/batch-outbound-transfers/outbound-macro-examples.md).
...
<table id="table_5C6F9678CFF34C5EB67BA1DEA0479F1D"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <code> CLOSE_COLLAGE_BRACKETS </code> </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>데이터 파트너에 대한 여러 ID가 포함된 목록을 반환합니다. 이 기능은 여러 하위 부구나 다른 조직 그룹이 있는 대규모 조직이 있는 경우 데이터를 공유할 수 있는 경우에 유용합니다. 이 매크로는 하위 그룹의 ID 목록을 반환합니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> DPID </code> </p> </td> 
   <td colname="col2"> <p>데이터 공급자 ID. </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>이 매크로의 출력은 데이터 공급자 ID(DPID)를 관련 DPUUID(사용자 ID)에 매핑합니다. 이 매크로에는 출력을 제어하기 위한 서식 문자열이 있어야 합니다. 샘플 출력은 다음과 비슷합니다. </p> <p> <code> "dpids=dpid1,dpid2,...dpid n|maxMappings= n|format=json" </code> </p> <p>최대 <code> 매핑 </code> 설정은 매크로를 반환할 매핑 수를 결정합니다. max <code> Mappings=0 </code>이면 이 매크로는 지정된 각 DPID에 대한 모든 매핑을 반환합니다. 데이터는 타임스탬프(가장 최근 우선)별로 정렬되고 가장 큰 타임스탬프가 있는 결과를 먼저 반환합니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> if(SEGMENT_LIST &amp;&amp; REMOVED_SEGMENT_LIST)endif </code> </p> </td> 
   <td colname="col2"> <p>이 매크로 조합은 사용자가 속하고 제거된 세그먼트를 나열하는 조건문을 만듭니다. 두 조건이 모두 충족되지 않거나 데이터가 없는 경우 빈 문자열을 반환합니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> <p> <code> OPEN_CURLY_BRACKETS </code> </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>정적 하드 코딩된 값으로 <code> 1을 </code> 반환합니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>목록의 세그먼트 목록을 반환합니다. 다음 선택적 인수를 수락합니다. </p> 
    <ul id="ul_B111AA0D6C18445598A1444B8B7E9325"> 
     <li id="li_8603B40229624856AF1FBC434DB8F16A"> <code> segmentId </code>:세그먼트 ID. 삭제 예정. sid <code> 를 </code>사용합니다. </li> 
     <li id="li_1EF40DDCA3C5447586904CF021D8F912"> <code> csegid </code>:고객 세그먼트 ID. 삭제 예정. sid <code> 를 </code>사용합니다. </li> 
     <li id="li_D85F0A5D16AE4DAFB55C17DBB35EA66E"> <code> sid </code>: 세그먼트 ID </li> 
     <li id="li_9BE103EFD8384464B46FAC00422431DB"> <code> type </code>:데이터를 세그먼트 데이터로 식별하는 정적 하드 코딩된 <code> 값인 5를 </code>반환합니다. </li> 
     <li id="li_FE5049089F2944FA9DB9F9D546DBA167"> <code> 별칭 </code>:가치 하락. 사용하지 마십시오. </li> 
     <li id="li_DD778AA2D1DB4D409CF5026B5D9DBD27"> <code> lastUpdateTime </code>:세그먼트가 마지막으로 실현된 시간을 나타내는 Unix 타임스탬프 </li> 
    </ul> <p>매크로 뒤에 중괄호로 이 변수를 배치합니다. 예를 들어 이 코드는 "|" 문자로 결과를 구분합니다.&lt; <code> SEGMENT_LIST:{seg|&lt;seg.type&gt;,&lt;seg.sid&gt;};separator=","&gt; </code> </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>동기화 유형을 나타내며 다음을 포함합니다. </p> 
    <ul id="ul_A3ADC37E66F043DABDA9C4066024B6C1"> 
     ...
     <li id="li_520DDED3662B428DB9DB55D494221D97"> <code> 로그 </code>:증분 동기화. </li> 
    </ul> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <code> SYNC_TYPE </code> </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>트레이트 목록을 반환합니다. 다음 선택적 인수를 수락합니다. </p> 
    <ul id="ul_E9CDC4DD47B9435086FF42143D9E8177"> 
     <li id="li_4BBC57F0D7874F8EA8C6D39DB3572257"> <code> type </code>:트레이트 유형을 숫자 ID로 식별합니다. 반환: 
      <ul id="ul_D2357E6CF47B4EBC8D3772D17B2EADA3"> 
       <li id="li_C6C2A019FCD945E085E1ABB564C4EDAD"> <code> DPM 트레이트를 식별하는 </code> 10(오프라인, 인바운드 작업에 의해 온보드) </li> 
       <li id="li_7AFF8A1D0E1140459CC95CF43A97B9B6"> <code> 3. 규칙 기반 트레이트를 식별하는 </code> 데, DCS를 통해 실시간으로 온보드) </li> 
      </ul> </li> 
     <li id="li_1DDE25334CF9479A8C4738F3CB3C40AA"> <code> traitId </code>:트레이트 ID. </li> 
     <li id="li_DCB89F2A40BB43C98EE3C84B5B3CDD33"> <code> 마지막 실현 </code>:마지막으로 트레이트가 실현된 시간입니다. Unix 타임스탬프 </li> 
    </ul> <p>매크로 뒤에 중괄호로 이 변수를 배치합니다. 예를 들어 이 코드는 "|" 문자로 결과를 구분합니다.&lt;TRAIT_LIST:{trait|&lt;trait.Id&gt;,&lt;trait.lastRealized&gt;};separator="," <code></code> </p> </td> 
  </tr> 
  <tr> 
