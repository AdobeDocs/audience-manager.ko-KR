---
description: 병합 규칙 옵션을 사용하면 세그멘테이션에 사용하는 데이터 Audience Manager의 유형을 제어할 수 있습니다. 병합 규칙에는 프로파일 링크 장치 그래프, Adobe Experience Cloud 장치 협력 및/또는 Audience Manager과 통합된 기타 타사 장치 그래프 공급자가 매핑하는 장치 프로파일이 포함될 수 있습니다. 최대 4개의 프로필 병합 규칙을 만들 수 있습니다.
seo-description: 병합 규칙 옵션을 사용하면 세그멘테이션에 사용하는 데이터 Audience Manager의 유형을 제어할 수 있습니다. 병합 규칙에는 프로파일 링크 장치 그래프, Adobe Experience Cloud 장치 협력 및/또는 Audience Manager과 통합된 기타 타사 장치 그래프 공급자가 매핑하는 장치 프로파일이 포함될 수 있습니다. 최대 4개의 프로필 병합 규칙을 만들 수 있습니다.
...
[!UICONTROL profile merge rule] 옵션을 사용하면 세그먼테이션에 사용되는 데이터 유형 [!DNL Audience Manager]을 제어할 수 있습니다. [!UICONTROL profile merge rule]은 [!UICONTROL Profile Link] 장치 그래프, [!UICONTROL Adobe Experience Cloud Device Co-op] 및/또는 [!DNL Audience Manager]과 통합된 타사 장치 그래프 공급자가 매핑하는 장치 프로파일을 포함할 수 있습니다. 최대 4개의 [!UICONTROL Profile Merge Rules]을(를) 만들 수 있습니다. 네 번째 [!UICONTROL Profile Merge Rule]은 [!UICONTROL People-Based Destinations] Add-on을 구입한 고객에게만 제공됩니다.

[!UICONTROL Profile Merge Rule Setup]에 아래 설명된 옵션 중에서 선택하여 [!UICONTROL Profile Merge Rule]을 만듭니다.

![profile-merge-rule-setup](assets/profile-merge-rule-setup.png)

## [!UICONTROL Profile Merge Rule] 옵션 개요  {#overview}

[!UICONTROL Profile Merge Rules] 특정 사용 사례에 맞게 여러 규칙 조합을 허용합니다. 각 규칙 조합을 사용할 시기에 대한 자세한 내용은 아래 표를 참조하십시오.
...
* 실시간 [!UICONTROL segment] 평가를 수행하려면 방문자가 디지털 속성에 실시간으로 액세스하는 것을 보려면 [!DNL DCS] 이 필요합니다. [!UICONTROL segment]
* 이전에 자격이 부여된 [!UICONTROL traits]에 대해 일괄 [!UICONTROL segment] 평가가 수행됩니다.
* [!UICONTROL Profile Merge Rules] 실시간 및 일괄  [!UICONTROL segment] 평가를 모두 지원하는 실시간 방문자 활동은 이전에 자격 조건을 갖춘 방문자 활동과 결합되어 있습니다 [!UICONTROL traits].
...
[!UICONTROL Cross-Device Options]을 사용하면 인증된 사용자와 인증되지 않은 사용자를 선택할 수 있고 세그먼테이션을 위해 상호 장치 프로파일을 활용할 수 있습니다. 이러한 옵션은 공유 장치의 특정 사용자를 식별하고 연결하는 데 도움이 됩니다. 익명 사용자와 인증된 사용자에 대한 자세한 내용은 [Audience Manager의 방문자 인증 상태](../../reference/visitor-authentication-states.md)를 참조하십시오.
...
<table id="table_4CE2DD312F54480E96BEAF72800789FB"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>방문자가 사이트에 로그인한 경우 <span class="keyword"> Audience Manager</span>에 데이터를 읽고 쓰기 지시합니다. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b><span class="uicontrol"> 마지막으로 인증된 프로필</span></b> </p> </td> 
   <td colname="col2"> <p>장치에 마지막으로 로그인한 사용자의 인증된 프로필에서 데이터를 읽도록 <span class="keyword"> Audience Manager</span>에 지시합니다. </p> <p>이 옵션을 선택하면 사용자가 익명 상태인 경우 <span class="keyword"> Audience Manager</span>은(는) 인증된 프로필에 새 특성 데이터를 기록하지 않습니다. 인증 시 새로운 트레이트 데이터가 사용자의 인증된 프로필에 기록됩니다. </p> </td>
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b><span class="uicontrol"> 모든 크로스 디바이스 프로파일</span></b> </p> </td> 
   <td colname="col2"> <p>Audience Manager에 인증 상태에 상관없이 모든 크로스 디바이스 프로파일에서 데이터를 읽도록 지시합니다. 이 옵션은 사용자 기반 대상 Add-On을 구입한 Audience Manager 고객에게만 제공됩니다.</p> </td>
  </tr>
 </tbody>
</table>
...
[!UICONTROL Device Options]에서는 [!UICONTROL Profile Merge Rule]에서 사용하는 *`device profile`* 유형을 선택할 수 있습니다. 장치 프로파일은 익명 탐색 활동에서 수집된 [!UICONTROL traits]에서 만들어집니다. 최소한 [!UICONTROL profile merge rule]에는 [!UICONTROL authenticated option] 및 [!UICONTROL device option]가 포함됩니다.
...
<table id="table_D373FB787D1A4E3485C02C4A76F03395"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>세그먼테이션을 위해 익명 프로필에 포함된 트레이트를 사용하지 않도록 <span class="keyword"> Audience Manager</span>에 알립니다. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p><span class="keyword"> Audience Manager</span>에 현재 장치에서 프로파일을 읽고 사용자가 인증한 다른 장치는 최대 100개까지 읽도록 지시합니다. 이 장치 그래프는 <span class="keyword"> Audience Manager</span>에 있는 자사 데이터를 기반으로 합니다. 디지털 자산에서 높은 수준의 인증을 보유한 고객에게 적합합니다. <span class="wintitle"> 프로필 링크</span> 장치 그래프가 실시간으로 업데이트됩니다. 이 옵션은 <b><span class="uicontrol"> 현재 인증된 프로필</span></b> 또는 <b><span class="uicontrol"> 마지막 인증된 프로필</span></b>을 선택하면 사용할 수 있습니다. 이 옵션을 사용할 때는 인증된 단일 프로필(<span class="keyword"> Audience Manager</span> 다른 프로필이 자동으로 수집됨)만 선택할 수 있습니다. 참조: <a href="profile-link-use-case.md"> 프로필 링크 장치 그래프 사용 사례</a>. </p> </td>
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b><span class="uicontrol"> Co-op 장치 그래프</span></b> </p> </td> 
   <td colname="col2"> <p><span class="keyword"> Audience Manager</span>에 <a href="https://docs.adobe.com/content/help/en/device-co-op/using/about/overview.html" format="https" scope="external"> Experience Cloud 장치 Co-op</a>에서 제공하는 링크를 사용하여 현재 장치에서 프로파일을 읽고 다른 장치를 최대 100개까지 읽도록 지시합니다. </p> <p><span class="keyword"> Device Co-op</span>는 참여하는 고객이 장치 링크 정보를 공유하는 디지털 협업입니다. <span class="keyword"> 장치 Co-op</span>은 <span class="term"> 장치 그래프</span>에서 이 데이터를 처리합니다. 장치 그래프는 장치 클러스터를 서로 연결합니다. 이러한 링크는 <a href="https://docs.adobe.com/content/help/en/device-co-op/using/device-graph/links.html" format="https" scope="external"> 확률적 및 결정론적 데이터</a>에서 만들어집니다. 클러스터는 알 수 없는 사람이 사용하는 장치 그룹을 나타냅니다. <span class="keyword">Device Co-op</span>는 그 구성원들 간에 이러한 클러스터를 공유하며, 이를 통해 고객에게 가치 있고 일관된 크로스 디바이스 환경을 제공할 수 있습니다. </p> <p> <span class="wintitle"> 장치 협력</span>에 대한 자세한 내용은 다음을 참조하십시오. </p> <p> 
  ...
  <tr> 
   <td colname="col1"> <p><b>제3자 장치 그래프 옵션</b> (개인 및 가구) </p> </td>
   <td colname="col2"> <p>이러한 옵션을 사용하면 타사 공급업체에서 제공하는 장치 그래프 기술을 기반으로 병합 규칙을 만들 수 있습니다. 제3자 장치 그래프는 다음을 제공합니다. </p> <p> 
     ...
     </ul> </p> <p>이러한 옵션을 사용하려면 이미 <span class="keyword"> Audience Manager</span>에 통합된 장치 그래프의 고객이어야 합니다. 자세한 내용을 확인하거나 시작하려면 계정 관리자에게 문의하십시오. </p> </td>
  </tr>
 </tbody>
</table>
